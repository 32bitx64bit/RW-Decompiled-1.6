...
public class Pawn_PathFollower : IExposable
{
	...
	public void DrawDebugGUI()
	{
		...
		{
			val = new Color(0.14f, 0.93f, 1f, 0.5f);
		}
		else if (pawn.stances.FullBodyBusy)
		{
			val = new Color(0.37f, 1f, 0.19f, 0.5f);
		}
		else if (Moving && WillCollideNextCell)
		{
			val = new Color(1f, 0.71f, 0.22f, 0.5f);
		}
		if (val.HasValue)
		{
			GUI.DrawTexture(adjustedScreenspaceRect, (Texture)(object)TexUI.DotHighlight, (ScaleMode)2, true, 0f, val.Value, 0f, 0f);
		}
	}
