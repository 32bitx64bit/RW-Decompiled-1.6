...
public class PathGrid : IDisposable
{
	...
	public PathGrid(Map map, PathGridDef def)
	{
		...
		this.map = map;
		this.def = def;
		pathGrid = new NativeArray<int>(map.cellIndices.NumGridCells, (Allocator)4, (NativeArrayOptions)1);
	}
	...
	public void RecalculatePerceivedPathCostAt(IntVec3 c, ref bool haveNotified)
	{
		...
		if (((NativeBitArray)(ref dirty)).IsCreated)
		{
			((NativeBitArray)(ref dirty)).Set(map.cellIndices.CellToIndex(c), true);
		}
	...
	private bool RecalcuateCellDirect(IntVec3 c)
	{
		...
		int num = map.cellIndices[c];
		bool num2 = WalkableFast(num);
		pathGrid[num] = CalculatedCostAt(c, perceivedStatic: true, IntVec3.Invalid);
		return num2 != WalkableFast(num);
	}
	...
	public void RecalculateAllPerceivedPathCosts()
	{
		...
		if (((NativeBitArray)(ref dirty)).IsCreated)
		{
			((NativeBitArray)(ref dirty)).Clear();
		}
	}
	...
	public void DisableIncrementalDirtying()
	{
		...
		if (!((NativeBitArray)(ref dirty)).IsCreated)
		{
			dirty = new NativeBitArray(map.cellIndices.NumGridCells, AllocatorHandle.op_Implicit((Allocator)4), (NativeArrayOptions)1);
		}
	}
	...
	public void ReEnableIncrementalDirtying()
	{
		...
		for (int i = 0; i < ((NativeBitArray)(ref dirty)).Length; i++)
		{
			if (((NativeBitArray)(ref dirty)).IsSet(i))
			{
		...
		((NativeBitArray)(ref dirty)).Dispose();
	}
