...
public class BreachingGrid : IExposable
{
	private class CustomTuning : PathRequest.IPathGridCustomizer, IDisposable
	{
		...
		public CustomTuning(BreachingGrid grid, PathFinderCostTuning tuning)
		{
			...
			this.grid = grid;
			this.tuning = tuning;
			offsets = new NativeArray<ushort>(grid.map.cellIndices.NumGridCells, (Allocator)3, (NativeArrayOptions)1);
			SetupOffsetGrid();
		}
		...
		private void SetupOffsetGrid()
		{
			...
			for (int i = 0; i < cellIndices.NumGridCells; i++)
			{
				...
				{
					ref NativeArray<ushort> reference = ref offsets;
					int num2 = i;
					reference[num2] += tweakPerlinCost;
				}
				...
				if (edifice != null && PathUtility.IsDestroyable(edifice))
				{
					ref NativeArray<ushort> reference = ref offsets;
					int num2 = i;
					reference[num2] += (ushort)((float)tuning.costBlockedWallBase + (float)edifice.HitPoints * tuning.costBlockedWallExtraPerHitPoint);
					reference = ref offsets;
					num2 = i;
					reference[num2] += (ushort)((!edifice.def.IsBuildingArtificial) ? tuning.costBlockedWallExtraForNaturalWalls : 0);
				}
			}
		}
	}
