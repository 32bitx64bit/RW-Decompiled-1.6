		...
		for (int i = 0; i < reservations.Count; i++)
		{
			...
			if (reservation.Target.Thing != null)
			{
				if (reservation.Target.Thing.Spawned)
				{
					...
					((Vector3)(ref val))._002Ector((float)thing.RotatedSize.x, 1f, (float)thing.RotatedSize.z);
					Matrix4x4 val2 = default(Matrix4x4);
					((Matrix4x4)(ref val2)).SetTRS(thing.DrawPos + Vector3.up * 0.1f, Quaternion.identity, val);
					Graphics.DrawMesh(MeshPool.plane10, val2, DebugReservedThingIcon, 0);
