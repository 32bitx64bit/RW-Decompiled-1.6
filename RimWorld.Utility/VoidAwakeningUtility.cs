...
public static class VoidAwakeningUtility
{
	...
	{
		float num = DefenderPointsByCombatPoints.Evaluate(StorytellerUtility.DefaultThreatPointsNow(structure.Map));
		num = Mathf.Max(num, Faction.OfEntities.def.MinPointsToGeneratePawnGroup(PawnGroupKindDefOf.Metalhorrors) * 1.05f);
