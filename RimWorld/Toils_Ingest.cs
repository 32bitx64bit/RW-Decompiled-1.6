...
public class Toils_Ingest
{
	...
	public static Toil CarryIngestibleToChewSpot(Pawn pawn, TargetIndex ingestibleInd)
	{
		...
		toil.initAction = delegate
		{
			...
			{
				Log.WarningOnce($"Can't find valid chair or spot for {actor} trying to ingest {thing}", HashCode.Combine<Pawn, Thing>(actor, thing));
	...
	public static Toil ReserveFoodFromStackForIngesting(TargetIndex ind, Pawn ingester = null)
	{
		...
		toil.initAction = delegate
		{
			...
			{
				int num2 = FoodUtility.WillIngestStackCountOf(ingester, target.Thing.def, FoodUtility.NutritionForEater(ingester, target.Thing));
				num = Mathf.Min(target.Thing.stackCount, num2);
			}
