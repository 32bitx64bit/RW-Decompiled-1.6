...
public static class MapGenUtility
{
	...
	internal static class ComputeLargestRects_0000B6FE_0024BurstDirectCall
	{
		...
		private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(ComputeLargestRects_0000B6FE_0024PostfixBurstDelegate).TypeHandle);
			}
			P_0 = Pointer;
		}
		...
		static ComputeLargestRects_0000B6FE_0024BurstDirectCall()
		{
			Constructor();
		}
	...
	public delegate void RectsComputeSpaces_0000B6FF_0024PostfixBurstDelegate(ref NativeArray<CellData> set, in NativeArray<float> elevation, in CellIndices indices, float minElevation, float maxElevation, int borderPadding);

	internal static class RectsComputeSpaces_0000B6FF_0024BurstDirectCall
	{
		private static IntPtr Pointer;
		...
		private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(RectsComputeSpaces_0000B6FF_0024PostfixBurstDelegate).TypeHandle);
			}
			P_0 = Pointer;
		}
		...
		static RectsComputeSpaces_0000B6FF_0024BurstDirectCall()
		{
			Constructor();
		}
	...
	public static bool TryGetLargestClearRect(out CellRect rect, Func<CellRect, bool> rectValidator = null, float minElevation = -1f, float maxElevation = 0.7f, int mapBorderPadding = 8)
	{
		...
		if (clearRects.Any())
		{
			rect = GenCollection.MaxBy(clearRects, (CellRect r) => r.Area);
			return true;
		}
		rect = CellRect.Empty;
		return false;
	...
	public static bool TryGetClosestClearRectTo(out CellRect rect, IntVec2 size, IntVec3 position, Func<CellRect, bool> rectValidator = null, float minElevation = -1f, float maxElevation = 0.7f, int mapBorderPadding = 8)
	{
		...
		if (!GenCollection.MinBy(clearRects, (CellRect r) => r.CenterCell.DistanceToSquared(position)).TryFindNearestInnerRectTo(size, position, out var rect2))
		{
			rect = CellRect.Empty;
			return false;
		}
	...
	public static List<CellRect> GetClearRects(int minWidth, int minHeight, int maxWidth = -1, int maxHeight = -1, float minElevation = -1f, float maxElevation = 0.7f, int mapBorderPadding = 8)
	{
		...
		UnsafeList<CellRect> rects = default(UnsafeList<CellRect>);
		rects._002Ector(12, AllocatorHandle.op_Implicit((Allocator)4), (NativeArrayOptions)0);
		NativeArray<CellData> set = default(NativeArray<CellData>);
		set._002Ector(indices.NumGridCells, (Allocator)4, (NativeArrayOptions)1);
	...
	{
		ComputeLargestRects_0000B6FE_0024BurstDirectCall.Invoke(ref rects, ref set, in elevation, in indices, minWidth, minHeight, maxWidth, maxHeight, minElevation, maxElevation, mapBorderPadding);
	}
	...
	{
		RectsComputeSpaces_0000B6FF_0024BurstDirectCall.Invoke(ref set, in elevation, in indices, minElevation, maxElevation, borderPadding);
	}
	...
	public static void SpawnExteriorLumps(Map map, ThingDef thingDef, FloatRange countPer10K, IntRange countRange, IntRange distRange, int maxDistFromBuilding = 6)
	{
		...
		foreach (LayoutStructureSketch layoutStructureSketch in map.layoutStructureSketches)
		{
			foreach (LayoutRoom room2 in layoutStructureSketch.structureLayout.Rooms)
			{
				hashSet.RemoveWhere((IntVec3 c) => room2.Contains(c, 1));
			}
		}
		...
		while (true)
		{
			...
			for (int i = -num; i <= num; i++)
			{
				for (int j = -num; j <= num; j++)
				{
					int num9 = num2 + i;
					int num10 = num3 + j;
					if (Vector2.SqrMagnitude(new Vector2((float)num2, (float)num3) - new Vector2((float)num9, (float)num10)) <= (float)(num * num))
					{
				...
				if (num3 >= minWidth && num2 >= minHeight)
				{
					...
					if (!flag)
					{
						for (int num4 = c.z - 1; num4 >= c.z - num2; num4--)
						{
							...
							int num5 = indices.CellToIndex(c2);
							CellData cellData = set[num5];
						...
						if (num3 >= minWidth && num2 >= minHeight)
						{
							...
							CellRect other = new CellRect(c.x - num3 + 1, c.z - num2 + 1, num3, num2);
							bool flag2 = false;
							for (int j = 0; j < rects.Length; j++)
							{
								if (rects[j].Overlaps(other))
								{
									if (other.Area <= rects[j].Area)
									{
										flag = true;
										break;
									}
							...
							if (!flag)
							{
								if (flag2)
								{
									for (int num6 = rects.Length - 1; num6 >= 0; num6--)
									{
										if (rects[num6].Overlaps(other))
										{
											rects.RemoveAt(num6);
										}
									}
	...
	public static void RectsComputeSpaces_0024BurstManaged(ref NativeArray<CellData> set, in NativeArray<float> elevation, in CellIndices indices, float minElevation, float maxElevation, int borderPadding)
	{
		...
		for (int i = 0; i < indices.SizeX; i++)
		{
			...
			for (int j = 0; j < indices.SizeZ; j++)
			{
				...
				int num2 = indices.CellToIndex(c);
				float num3 = elevation[num2];
				if (num3 <= minElevation || num3 >= maxElevation || c.x < borderPadding || c.z < borderPadding || c.x > indices.SizeX - borderPadding || c.z > indices.SizeZ - borderPadding)
				{
					set[num2] = new CellData
					{
						x = -1,
						z = -1
					};
				...
				int num4 = indices.CellToIndex(c2);
				int x = 1;
				if (indices.Contains(c2) && set[num4].x > 0)
				{
					x = set[num4].x + 1;
				}
				set[num2] = new CellData
