...
public class Building_TurretGun : Building_Turret
{
	...
	public override void DrawExtraSelectionOverlays()
	{
		...
		{
			Vector3 val = ((!forcedTarget.HasThing) ? forcedTarget.Cell.ToVector3Shifted() : forcedTarget.Thing.TrueCenter());
			Vector3 a = this.TrueCenter();
			val.y = AltitudeLayer.MetaOverlays.AltitudeFor();
			a.y = val.y;
			GenDraw.DrawLineBetween(a, val, ForcedTargetLineMat);
		}
	}
	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		if (CanExtractShell)
		{
			...
			command_Action.icon = (Texture)(object)compChangeableProjectile.LoadedShell.uiIcon;
		...
		if (!HideForceTargetGizmo)
		{
			if (CanSetForcedTarget)
			{
				...
				command_VerbTarget.icon = (Texture)(object)ContentFinder<Texture2D>.Get("UI/Commands/Attack");
			...
			if (forcedTarget.IsValid)
			{
				...
				command_Action2.icon = (Texture)(object)ContentFinder<Texture2D>.Get("UI/Commands/Halt");
		...
		command_Toggle.icon = (Texture)(object)ContentFinder<Texture2D>.Get("UI/Commands/HoldFire");
