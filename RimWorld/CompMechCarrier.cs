...
public class CompMechCarrier : ThingComp, IThingHolder
{
	...
	public int MaxCanSpawn => Mathf.Min(Mathf.FloorToInt((float)(IngredientCount / Props.costPerPawn)), Props.maxPawnsToSpawn);
	...
	public override IEnumerable<Gizmo> CompGetGizmosExtra()
	{
		...
		{
			cooldownPercentGetter = () => Mathf.InverseLerp((float)Props.cooldownTicks, 0f, (float)cooldownTicksRemaining),
			action = delegate
			{
				TrySpawnPawns();
			},
			...
			icon = (Texture)(object)ContentFinder<Texture2D>.Get("UI/Gizmos/ReleaseWarUrchins"),
