...
public class ThingSetMaker_RandomGeneralGoods : ThingSetMaker
{
	...
	private Thing RandomMedicine(TechLevel techLevel)
	{
		...
		{
			result = GenCollection.MaxBy(ThingSetMakerUtility.allGeneratableItems.Where((ThingDef x) => x.IsMedicine && (int)x.techLevel <= (int)techLevel), (ThingDef x) => x.GetStatValueAbstract(StatDefOf.MedicalPotency));
		}
