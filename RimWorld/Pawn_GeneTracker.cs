...
public class Pawn_GeneTracker : IExposable
{
	...
	public float BiologicalAgeTickFactor
	{
		get
		{
			...
			bool valueOrDefault = cachedGenesAffectAge.GetValueOrDefault();
	...
	private void Notify_GenesChanged(GeneDef addedOrRemovedGene)
	{
		...
		{
			Color val = chosen3.def.hairColorOverride.Value;
			if (chosen3.def.randomBrightnessFactor != 0f)
			{
				val *= 1f + Rand.Range(0f - chosen3.def.randomBrightnessFactor, chosen3.def.randomBrightnessFactor);
			}
			pawn.story.HairColor = val.ClampToValueRange(GeneTuning.HairColorValueRange);
		...
		if (ModLister.BiotechInstalled)
		{
			if (addedOrRemovedGene.skinColorOverride.HasValue)
			{
				...
				{
					Color val2 = chosen5.def.skinColorOverride.Value;
					if (chosen5.def.randomBrightnessFactor != 0f)
					{
						val2 *= 1f + Rand.Range(0f - chosen5.def.randomBrightnessFactor, chosen5.def.randomBrightnessFactor);
					}
					pawn.story.skinColorOverride = val2.ClampToValueRange(GeneTuning.SkinColorValueRange);
