...
public class Plant : ThingWithComps
{
	...
	private static Color32[] workingColors = (Color32[])(object)new Color32[4];
	...
	public override void TickLong()
	{
		...
		if (def.plant.dropLeaves && MoteMaker.MakeStaticMote(Vector3.zero, base.Map, ThingDefOf.Mote_Leaf) is MoteLeaf moteLeaf)
		{
			...
			Vector3 val = Rand.InsideUnitCircleVec3 * LeafSpawnRadius;
			moteLeaf.Initialize(base.Position.ToVector3Shifted() + Vector3.up * Rand.Range(LeafSpawnYMin, LeafSpawnYMax) + val + Vector3.forward * def.graphicData.shadowData.offset.z, Rand.Value * 2000.TicksToSeconds(), val.z > 0f, treeHeight);
		}
	}
	...
	public override void Print(SectionLayer layer)
	{
		...
		foreach (int num6 in positionIndices)
		{
			...
			{
				val2 = val + Gen.RandomHorizontalVector(0.05f);
				float num7 = base.Position.z;
				if (val2.z - num2 / 2f < num7)
				{
					val2.z = num7 + num2 / 2f;
					flag2 = true;
				}
			...
			else
			{
				...
				val2 = base.Position.ToVector3();
				val2.y = def.Altitude;
				val2.x += 0.5f * num9;
				val2.z += 0.5f * num9;
				int num10 = num6 / num8;
				int num11 = num6 % num8;
				val2.x += (float)num10 * num9;
				val2.z += (float)num11 * num9;
				float max = num9 * 0.3f;
				val2 += Gen.RandomHorizontalVector(max);
			}
			bool @bool = Rand.Bool;
			object obj;
			if (flag)
			...
			Graphic.TryGetTextureAtlasReplacementInfo(material2, def.category.ToAtlasGroup(), @bool, vertexColors: false, out material2, out var uvs, out var vertexColor);
			PlantUtility.SetWindExposureColors(workingColors, this);
			((Vector2)(ref size))._002Ector(num3, num3);
			Printer_Plane.PrintPlane(layer, val2, size, material2, 0f, @bool, uvs, workingColors, 0.1f, this.HashOffset() % 1024);
			if (flag && (Object)(object)material != (Object)null)
			{
				Graphic.TryGetTextureAtlasReplacementInfo(material, def.category.ToAtlasGroup(), @bool, vertexColors: false, out material, out uvs, out vertexColor);
				Printer_Plane.PrintPlane(layer, val2.WithYOffset(0.0003658537f), size, material, 0f, @bool, uvs, workingColors, 0.1f, this.HashOffset() % 1024);
			}
			num4++;
			if (num4 >= num)
			{
		...
		{
			Vector3 center = val + def.graphicData.shadowData.offset * num2;
			if (flag2)
			{
		...
		if (def.plant.showGrowthInInspectPane)
		{
			if (LifeStage == PlantLifeStage.Growing)
			{
				...
				if (!Blighted)
				{
					...
					ArrayPool<string>.Shared.Return(array, false);
					if (!text.NullOrEmpty())
					{
		...
		if (Blighted)
		{
			...
			if (designation == null || designation.def != DesignationDefOf.CutPlant)
			{
				yield return new Command_Action
				{
					...
					icon = (Texture)(object)CutAllBlightTex,
					action = delegate
					{
