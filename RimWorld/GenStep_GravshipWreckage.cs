...
public class GenStep_GravshipWreckage : GenStep
{
	...
	private static void SpawnCraters(Map map, IntVec3 root)
	{
		...
		{
			ThingDef def2 = source.RandomElement();
			if (RCellFinder.TryFindRandomCellNearWith(root, (IntVec3 c) => Validator(c, def2), map, out var result, 7, 40))
			{
				GenSpawn.Spawn(def2, result, map);
				continue;
			}
			break;
		}
	...
	private static void SpawnFilth(Map map, IntVec3 root)
	{
		...
		for (int i = 0; i < randomInRange; i++)
		{
			...
			if (RCellFinder.TryFindRandomCellNearWith(root, (IntVec3 c) => Validator(c, def2), map, out var result, 7, 40))
			{
				FilthMaker.TryMakeFilth(result, map, def2);
				continue;
			}
			break;
		}
	...
	{
		float num = Faction.OfMechanoids.def.MinPointsToGeneratePawnGroup(PawnGroupKindDefOf.Combat);
		PawnGroupMakerParms pawnGroupMakerParms = new PawnGroupMakerParms
		{
			groupKind = PawnGroupKindDefOf.Combat,
			tile = map.Tile,
			faction = Faction.OfMechanoids,
			points = Mathf.Max(points, num)
		};
