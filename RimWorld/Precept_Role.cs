...
public abstract class Precept_Role : Precept
{
	...
	public override List<PreceptApparelRequirement> GenerateNewApparelRequirements(FactionDef generatingFor = null)
	{
		...
		{
			List<PreceptApparelRequirement> list2 = AllPossibleRequirements(ideo, def, desperate);
			List<PreceptApparelRequirement> list3 = new List<PreceptApparelRequirement>(list2);
			foreach (PreceptApparelRequirement req in list2)
			{
			...
			ChooseApparelRequirements(list3);
			if (apparelRequirements.Count < apparelRequirementCount)
			{
	...
	protected override void PostDrawBox(Rect rect, out bool anyTooltipActive)
	{
		...
		if (!apparelRequirements.NullOrEmpty())
		{
			...
			num += (list.Count - 1) * 2;
			GenUI.DrawElementStack(new Rect(((Rect)(ref rect)).xMax - (float)num, ((Rect)(ref rect)).yMin, (float)num, 24f), 24f, list, delegate(Rect r, ThingDef apparel)
			{
				...
				GUI.DrawTexture(r, (Texture)(object)apparel.uiIcon, (ScaleMode)2, true, 0f, apparel.uiIconColor, 0f, 0f);
