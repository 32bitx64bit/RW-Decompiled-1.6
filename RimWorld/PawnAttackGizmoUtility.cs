...
public static class PawnAttackGizmoUtility
{
	...
	private static Gizmo GetSquadAttackGizmo(Pawn pawn)
	{
		...
		command_Target.icon = (Texture)(object)TexCommand.SquadAttack;
		...
		{
			foreach (Pawn item in Find.Selector.SelectedObjects.Where((object x) => x is Pawn pawn3 && CanOrderPlayerPawn(pawn3) && pawn3.Drafted).Cast<Pawn>())
			{
				string failStr2;
	...
	public static Gizmo GetMeleeAttackGizmo(Pawn pawn)
	{
		...
		command_Target.icon = (Texture)(object)TexCommand.AttackMelee;
