		...
		if (!Props.hideGizmosIfNotPlayerFaction || parent.Faction == Faction.OfPlayer)
		{
			...
			else
			{
				if (Props.targetFuelLevelConfigurable)
				{
					...
					command_SetTargetFuelLevel.icon = (Texture)(object)SetTargetFuelLevelCommand;
				...
				if (Props.showAllowAutoRefuelToggle)
				{
					...
					command_Toggle.icon = (Texture)(object)(allowAutoRefuel ? TexCommand.ForbidOn : TexCommand.ForbidOff);
