...
public static class StorytellerUI
{
	...
	public static void DrawStorytellerSelectionInterface(Rect rect, ref StorytellerDef chosenStoryteller, ref DifficultyDef difficulty, ref Difficulty difficultyValues, Listing_Standard infoListing)
	{
		...
		((Rect)(ref outRect))._002Ector(0f, 0f, Storyteller.PortraitSizeTiny.x + 16f, ((Rect)(ref rect)).height);
		Rect viewRect = default(Rect);
		((Rect)(ref viewRect))._002Ector(0f, 0f, Storyteller.PortraitSizeTiny.x, (float)DefDatabase<StorytellerDef>.AllDefs.Count() * (Storyteller.PortraitSizeTiny.y + 10f));
		Widgets.BeginScrollView(outRect, ref scrollPosition, viewRect);
		Rect val = GenUI.ContractedBy(new Rect(0f, 0f, Storyteller.PortraitSizeTiny.x, Storyteller.PortraitSizeTiny.y), 4f);
		foreach (StorytellerDef item in DefDatabase<StorytellerDef>.AllDefs.OrderBy((StorytellerDef tel) => tel.listOrder))
		{
			if (item.listVisible)
			{
				...
				Widgets.DrawOptionBackground(val, flag);
				if (Widgets.ButtonImage(val, item.portraitTinyTex, Color.white, new Color(0.72f, 0.68f, 0.59f)))
				{
				...
				if (flag)
				{
					GUI.DrawTexture(val, (Texture)(object)StorytellerHighlightTex);
				}
				((Rect)(ref val)).y = ((Rect)(ref val)).y + (((Rect)(ref val)).height + 8f);
			}
		}
		Widgets.EndScrollView();
		Rect outRect2 = default(Rect);
		((Rect)(ref outRect2))._002Ector(((Rect)(ref outRect)).xMax + 8f, 0f, ((Rect)(ref rect)).width - ((Rect)(ref outRect)).width - 8f, ((Rect)(ref rect)).height);
		((Rect)(ref explanationInnerRect)).width = ((Rect)(ref outRect2)).width - 16f;
		...
		((Rect)(ref rect2))._002Ector(0f, 120f, 290f, 9999f);
		float num = 300f;
		if (chosenStoryteller != null && chosenStoryteller.listVisible)
		{
			Rect val2 = default(Rect);
			((Rect)(ref val2))._002Ector(390f - ((Rect)(ref outRect2)).x, ((Rect)(ref rect)).height - Storyteller.PortraitSizeLarge.y - 1f, Storyteller.PortraitSizeLarge.x, Storyteller.PortraitSizeLarge.y);
			GUI.DrawTexture(val2, (Texture)(object)chosenStoryteller.portraitLargeTex);
			...
			foreach (DifficultyDef allDef in DefDatabase<DifficultyDef>.AllDefs)
			{
				...
				if (infoListing.RadioButton(labelCap, difficulty == allDef, 0f, allDef.description.ResolveTags(), 0f))
				{
					...
					else if (allDef != difficulty)
					{
						...
						float num2 = 0.6f;
						explanationScrollPositionAnimated = AnimationCurve.EaseInOut(time, explanationScrollPosition.y, time + num2, ((Rect)(ref explanationInnerRect)).height);
					}
					difficulty = allDef;
				}
				infoListing.Gap(3f);
			...
			num = ((Rect)(ref rect2)).y + infoListing.CurHeight;
			infoListing.End();
			if (difficulty != null && difficulty.isCustom)
			{
				if (explanationScrollPositionAnimated != null)
				{
					...
					Keyframe val3 = explanationScrollPositionAnimated.keys.Last();
					if (time2 < ((Keyframe)(ref val3)).time)
					{
				...
				float num3 = ((Rect)(ref val2)).xMax - ((Rect)(ref explanationInnerRect)).x;
				listing_Standard.ColumnWidth = num3 / 2f - 17f;
				Rect rect3 = default(Rect);
				((Rect)(ref rect3))._002Ector(0f, Math.Max(((Rect)(ref val2)).yMax, num), num3, 9999f);
				...
				float gapHeight = ((Rect)(ref outRect2)).height / 2f;
				...
				listing_Standard.Gap(gapHeight);
				num = ((Rect)(ref rect3)).y + listing_Standard.MaxColumnHeightSeen;
				listing_Standard.End();
			}
		}
		((Rect)(ref explanationInnerRect)).height = num;
		Widgets.EndScrollView();
		Widgets.EndGroup();
	}
