...
public class Building_SubcoreScanner : Building_Enterable, IThingHolderWithDrawnPawn, IThingHolder
{
	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		if (!initScanner)
		{
			...
			command_Action.icon = (Texture)(object)InitScannerIcon.Texture;
		...
		else if (base.SelectedPawn == null)
		{
			...
			command_Action2.icon = (Texture)(object)InsertPersonIcon.Texture;
			command_Action2.action = delegate
			{
				...
				for (int j = 0; j < allPawnsSpawned.Count; j++)
				{
					...
					if (!acceptanceReport.Accepted)
					{
						...
						{
							list.Add(new FloatMenuOption(pawn.LabelShortCap + ": " + acceptanceReport.Reason, (Action)null, (Thing)pawn, Color.white, MenuOptionPriority.Default, (Action<Rect>)null, (Thing)null, 0f, (Func<Rect, bool>)null, (WorldObject)null, playSelectionSound: true, 0));
						}
					}
					else
					{
						list.Add(new FloatMenuOption(pawn.LabelShortCap, (Action)delegate
						{
		...
		if (initScanner)
		{
			...
			command_Action3.icon = (Texture)(object)CancelLoadingIcon;
