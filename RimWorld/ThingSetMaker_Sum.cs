	...
	protected override void Generate(ThingSetMakerParams parms, List<Thing> outThings)
	{
		...
		for (int i = 0; i < optionsInResolveOrder.Count; i++)
		{
			...
			if (parms2.totalMarketValueRange.HasValue)
			{
				...
				{
					parms2.totalMarketValueRange = new FloatRange(Mathf.Min(parms2.totalMarketValueRange.Value.min, optionsInResolveOrder[i].maxMarketValue.Value * 0.8f), Mathf.Min(new float[1] { Mathf.Min(parms2.totalMarketValueRange.Value.max, optionsInResolveOrder[i].maxMarketValue.Value) }));
				}
