...
public class Ideo : IExposable, ILoadReferenceable
{
	...
	public Color Color
	{
		get
		{
			...
			if (!classicMode)
			{
				return (Color)(((_003F?)primaryFactionColor) ?? colorDef?.color ?? Color.white);
			}
			return Color.white;
		}
	}
	...
	public Color ApparelColor
	{
		get
		{
			...
			return Color.HSVToRGB(num, Mathf.Min(0.5f, num2), Mathf.Min(0.8235294f, num3));
		}
	}
	...
	public Color TextColor
	{
		get
		{
			...
			float num3 = default(float);
			Color.RGBToHSV(Color, ref num, ref num2, ref num3);
			return Color.HSVToRGB(num, num2, Mathf.Max(0.5f, num3));
		}
	}
	...
	public IdeoDescriptionResult GetNewDescription(bool force = false)
	{
		List<IdeoDescriptionMaker.PatternEntry> list = (from entry in memes.Where((MemeDef meme) => meme.descriptionMaker?.patterns != null).SelectMany((MemeDef meme) => meme.descriptionMaker.patterns)
			...
			select GenCollection.MaxBy(grp, (IdeoDescriptionMaker.PatternEntry entry) => entry.weight)).ToList();
		if (!list.Any())
		{
