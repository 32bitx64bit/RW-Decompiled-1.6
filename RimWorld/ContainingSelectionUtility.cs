...
public static class ContainingSelectionUtility
{
	...
	private static Gizmo CreateSelectGizmo(string label, string description, Thing thingToSelect, Thing iconThing = null)
	{
		...
		Texture icon;
		if (!(iconThing is Pawn pawn) || !((Object)(object)pawn.def.uiIcon == (Object)null))
		{
			icon = (Texture)((iconThing == null) ? ((object)((thingToSelect is Pawn) ? TexCommand.SelectCarriedPawn : TexCommand.SelectCarriedThing)) : ((object)Widgets.GetIconFor(iconThing, new Vector2(75f, 75f), iconThing.def.defaultPlacingRot, stackOfOne: true, out scale, out angle, out iconProportions, out color, out var _)));
		}
		else
		{
			...
			icon = (Texture)(object)PortraitsCache.Get(pawn, size, south, default(Vector3), 1f, supersample: true, compensateForUIScale: true, renderHeadgear: true, renderClothes: true, null, null, stylingStation: false, healthStateOverride);
		}
