...
public static class AbstractShapeGenerator
{
	...
	private static void GenerateInt(bool[,] grid, bool horizontalSymmetry, bool verticalSymmetry, bool allowEnclosedFalses, bool preferOutlines, float wipedCircleRadiusPct)
	{
		...
		for (int num13 = 0; num13 < num4; num13++)
		{
			...
			foreach (IntVec3 item3 in CellRect.CenteredOn(new IntVec3(Rand.RangeInclusive(0, length - 1), 0, Rand.RangeInclusive(0, length2 - 1)), (!bool2) ? 1 : Mathf.RoundToInt((float)Rand.RangeInclusive(1, length)), bool2 ? 1 : Mathf.RoundToInt((float)Rand.RangeInclusive(1, length2))))
			{
