...
public class MultiPawnGotoController
{
	...
	public void Draw()
	{
		...
		((Vector3)(ref val))._002Ector(1.7f, 1f, 1.7f);
		...
		for (int i = 0; i < pawns.Count; i++)
		{
			...
			if (c.IsValid && pawn.Spawned && !c.Fogged(pawn.Map))
			{
				...
				Vector3 val2 = c.ToVector3ShiftedWithAltitude(addedAltitude);
				Graphics.DrawMesh(MeshPool.plane10, Matrix4x4.TRS(val2, Quaternion.identity, val), GotoCircleMaterial, 0);
			}
		}
	...
	public void OnGUI()
	{
		...
		for (int i = 0; i < pawns.Count; i++)
		{
			...
			{
				Rect val = c.ToUIRect();
				((Vector2)(ref pos))._002Ector(((Rect)(ref val)).center.x, ((Rect)(ref val)).yMax + 5f);
