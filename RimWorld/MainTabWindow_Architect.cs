...
public class MainTabWindow_Architect : MainTabWindow
{
	...
	private void DoCategoryButton(ArchitectCategoryTab panel, float butWidth, float curXInd, float curYInd, ArchitectCategoryTab openTab, bool enabled)
	{
		...
		((Rect)(ref rect))._002Ector(curXInd * butWidth, curYInd * 32f, butWidth, 32f);
		float height = ((Rect)(ref rect)).height;
		((Rect)(ref rect)).height = height + 1f;
		if (curXInd == 0f)
		{
			((Rect)(ref rect)).width = ((Rect)(ref rect)).width + 1f;
		}
		Color? labelColor = (panel.AnySearchMatches ? null : new Color?(NoMatchColor));
	...
	public override void DoWindowContents(Rect inRect)
	{
		...
		float butWidth = ((Rect)(ref inRect)).width / 2f;
		float num = 0f;
		...
		float num2 = ((Rect)(ref inRect)).width;
		if (flag)
		{
			num2 -= 32f;
		}
		Rect rect = default(Rect);
		((Rect)(ref rect))._002Ector(0f, num * 32f + 1f, num2, 24f);
		...
		if (flag && Widgets.ButtonImage(GenUI.ContractedBy(new Rect(((Rect)(ref rect)).xMax + 4f, ((Rect)(ref rect)).y, 24f, 24f), 2f), ChangeStyleIcon.Texture))
		{
