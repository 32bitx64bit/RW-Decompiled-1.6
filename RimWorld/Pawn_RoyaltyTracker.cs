...
public class Pawn_RoyaltyTracker : IExposable
{
	...
	public IEnumerable<Gizmo> GetGizmos()
	{
		...
		if (faction != null)
		{
			...
			command_Action.icon = (Texture)(object)faction.def.FactionIcon;
	...
	public Gizmo RoyalAidGizmo()
	{
		...
		command_Action.icon = (Texture)(object)CommandTex;
	...
	public void UpdateAvailableAbilities()
	{
		...
		{
			for (int k = 0; k < AllTitlesInEffectForReading.Count; k++)
			{
				if (AllTitlesInEffectForReading[k].def.grantedAbilities.Contains(a.def))
				{
					return false;
				}
			}
		...
		for (int i = 0; i < AllTitlesInEffectForReading.Count; i++)
		{
			...
			for (int j = 0; j < royalTitle.def.grantedAbilities.Count; j++)
			{
				...
				{
					abilities.Add(AbilityUtility.MakeAbility(royalTitle.def.grantedAbilities[j], pawn));
				}
			}
		}
	}
