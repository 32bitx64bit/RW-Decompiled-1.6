...
public class Skyfaller : ThingWithComps, IThingHolder, IRoofCollapseAlert
{
	...
	protected Material ShadowMaterial
	{
		get
		{
			if ((Object)(object)cachedShadowMaterial == (Object)null && !def.skyfaller.shadow.NullOrEmpty())
			{
	...
	public static void DrawDropSpotShadow(Vector3 center, Rot4 rot, Material material, Vector2 shadowSize, int ticksToImpact)
	{
		...
		((Vector3)(ref val2))._002Ector(num * shadowSize.x, 1f, num * shadowSize.y);
		...
		((Matrix4x4)(ref val3)).SetTRS(val, rot.AsQuat, val2);
		Graphics.DrawMesh(MeshPool.plane10Back, val3, material, 0, (Camera)null, 0, shadowPropertyBlock);
	}
