...
public class MainTabsRoot
{
	...
	public void HandleLowPriorityShortcuts()
	{
		...
		if (Find.Selector.NumSelected == 0 && (int)Event.current.type == 0 && Event.current.button == 1 && !WorldRendererUtility.WorldSelected && (!SteamDeck.IsSteamDeck || !Input.GetMouseButton(2)))
		{
			Event.current.Use();
			MainButtonDefOf.Architect.Worker.InterfaceTryActivate();
		}
		if (OpenTab != null && OpenTab != MainButtonDefOf.Inspect && (int)Event.current.type == 0 && Event.current.button != 2)
		{
			EscapeCurrentTab();
