...
public class TileMutatorWorker_AncientStructure : TileMutatorWorker
{
	...
	private void GeneratePerimeter(LayoutStructureSketch layoutStructureSketch, Map map)
	{
		...
		{
			int num2 = ((!(parms.groupCount != IntRange.Invalid)) ? Mathf.RoundToInt(parms.groupsPerHundredCells.RandomInRange * num) : parms.groupCount.RandomInRange);
			num2 = Mathf.Max(num2, parms.minGroups);
			for (int i = 0; i < num2; i++)
			{
				...
				{
					IntVec3 c2 = list[j];
					if (PerimeterValidator(c2, parms.def) && Rand.DynamicChance(num3, randomInRange2, list.Count - j))
					{
						num3++;
