...
public class MonumentMarker : Thing
{
	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		if (!AllDone)
		{
			...
			command_Action.icon = (Texture)(object)CancelCommandTex;
		...
		if (flag)
		{
			...
			command_Action2.icon = (Texture)(object)PlaceBlueprintsCommandTex;
	...
	private Command_Action GetPlaceBlueprintsCommand(SketchBuildable buildable)
	{
		return new Command_Action
		{
			...
			icon = (Texture)(object)PlaceBlueprintsCommandTex,
			Order = 2000f,
			action = delegate
			{
