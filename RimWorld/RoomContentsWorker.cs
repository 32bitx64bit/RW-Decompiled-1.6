...
public class RoomContentsWorker
{
	...
	private void TryScatterTerrain(Map map, LayoutRoom room)
	{
		...
		{
			int num2 = ((!(item.groupCount != IntRange.Invalid)) ? Mathf.RoundToInt(item.groupsPerHundredCells.RandomInRange * num) : item.groupCount.RandomInRange);
			num2 = Mathf.Max(num2, item.minGroups);
	...
	private void TryPlacePrefabs(Map map, LayoutRoom room, Faction faction)
	{
		...
		{
			list.Add(item);
			int num3 = ((!(item.countRange != IntRange.Invalid)) ? ((!item.def.edgeOnly) ? Mathf.RoundToInt(item.countPerHundredCells.RandomInRange * num) : Mathf.RoundToInt(item.countPerTenEdgeCells.RandomInRange * num2)) : item.countRange.RandomInRange);
			num3 = Mathf.Min(Mathf.Max(num3, item.minMaxRange.TrueMin), item.minMaxRange.TrueMax);
			dictionary[item] = num3;
		}
		int num4 = 0;
		while (dictionary.Any())
		{
			LayoutPrefabParms parms = list[num4];
			num4 = ++num4 % list.Count;
	...
	private void TryFillContents(Map map, LayoutRoom room, Faction faction)
	{
		...
		{
			LayoutFillEdgesParms parms2 = item;
			if (parms2.countRange != IntRange.Invalid)
			{
				RoomGenUtility.FillAroundEdges(parms2.def, parms2.countRange.RandomInRange, parms2.groupCountRange, room, map, Validator, null, stuff: parms2.stuff, padding: parms2.padding, contractedBy: parms2.contractedBy, avoidDoors: true, rotationDirectionOffset: parms2.rotOffset, spawnAction: null, faction: faction);
				continue;
			}
			RoomGenUtility.FillAroundEdges(parms2.def, parms2.groupsPerTenEdgeCells, parms2.groupCountRange, room, map, Validator, null, stuff: parms2.stuff, padding: parms2.padding, contractedBy: parms2.contractedBy, avoidDoors: true, rotationDirectionOffset: parms2.rotOffset, spawnAction: null, faction: faction);
			bool Validator(IntVec3 c, Rot4 rot, CellRect rect)
			{
				return IsValidFillEdgeCell(parms2, c, rot, room, map);
			}
		}
