...
public class Building_Door : Building
{
	...
	protected void DrawMovers(Vector3 drawPos, float offsetDist, Graphic graphic, float altitude, Vector3 drawScaleFactor, Graphic_Shadow shadowGraphic)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (i == 0)
			{
				((Vector3)(ref val))._002Ector(0f, 0f, (float)(-def.size.x));
			...
			val = rotation.AsQuat * val;
			Vector3 val3 = drawPos;
			val3.y = altitude;
			val3 += val * offsetDist;
			Graphics.DrawMesh(val2, Matrix4x4.TRS(val3, base.Rotation.AsQuat, new Vector3((float)def.size.x * drawScaleFactor.x, drawScaleFactor.y, (float)def.size.z * drawScaleFactor.z)), graphic.MatAt(base.Rotation, this), 0);
			shadowGraphic?.DrawWorker(val3, base.Rotation, def, this, 0f);
		}
	}
	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		if ((base.Faction == Faction.OfPlayer || base.Faction == null) && !AlwaysOpen)
		{
			...
			command_Toggle.icon = (Texture)(object)TexCommand.HoldOpen;
