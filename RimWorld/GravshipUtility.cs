...
public static class GravshipUtility
{
	...
	public static bool PlayerHasGravEngine()
	{
		...
		ProfilerMarker playerHasGravEngineMarker = PlayerHasGravEngineMarker;
		AutoScope val = ((ProfilerMarker)(ref playerHasGravEngineMarker)).Auto();
		try
		{
	...
	public static Gravship GenerateGravship(Building_GravEngine engine)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			int num = map.cellIndices.CellToIndex(intVec);
			map.terrainGrid.RemoveGravshipTerrainUnsafe(intVec, num);
			map.roofGrid.RemoveRoofUnsafe(num);
			map.areaManager.BuildRoof[num] = false;
			map.areaManager.NoRoof[num] = false;
			map.glowGrid.DirtyCell(intVec);
			NativeArray<float> depthGrid_Unsafe = map.snowGrid.DepthGrid_Unsafe;
			depthGrid_Unsafe[num] = 0f;
			NativeArray<float> depthGrid_Unsafe2 = map.sandGrid.DepthGrid_Unsafe;
			depthGrid_Unsafe2[num] = 0f;
	...
	public static bool TryFindSpotOnGravship(Pawn pawn, Building_GravEngine engine, out IntVec3 spot)
	{
		...
		TraverseParms traverseParms2 = TraverseParms.For(TraverseMode.PassDoors);
		Region region2 = ClosestShipRegion(pawn.Position, pawn.MapHeld, pawn, traverseParms2);
		if (region2 == null)
		{
			desperate2 = true;
			region2 = ClosestShipRegion(pawn.Position, pawn.MapHeld, pawn, traverseParms2, desperate: true);
		}
		...
		bool TryGetAllowedCellInRegion(Region region, Pawn pawn, out IntVec3 cell, bool desperate = false)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				if (IsOnboardGravship_NewTemp(randomCell, engine, pawn, desperate, respectAllowedAreas: false))
				{
					cell = randomCell;
					return true;
				}
			...
			{
				if (IsOnboardGravship_NewTemp(item, engine, pawn, desperate, respectAllowedAreas: false))
				{
					cell = item;
					return true;
				}
	...
	public static bool TryFindSpotOffGravship(Pawn pawn, Building_GravEngine engine, out IntVec3 spot)
	{
		...
		List<IntVec3> tmpWorkingCellList = new List<IntVec3>();
		Region region2 = ClosestNonShipRegion(pawn.Position, pawn.MapHeld, pawn, TraverseParms.For(pawn));
		if (region2 != null && TryGetAllowedCellInRegion(region2, pawn, out spot))
		{
			return true;
		}
		return false;
		Region ClosestNonShipRegion(IntVec3 root, Map map, Pawn pawn, TraverseParms traverseParms)
		{
			RegionType regionType = RegionType.Set_Passable;
			Region region3 = root.GetRegion(map, regionType);
			if (region3 == null)
		...
		bool TryGetAllowedCellInRegion(Region region, Pawn pawn, out IntVec3 cell)
		{
			cell = IntVec3.Invalid;
			for (int i = 0; i < 10; i++)
			{
				IntVec3 randomCell = region.RandomCell;
				if (!IsOnboardGravship_NewTemp(randomCell, engine, pawn, desperate: true, respectAllowedAreas: false))
				{
					cell = randomCell;
					return true;
				}
			...
			foreach (IntVec3 item in region.Cells.InRandomOrder(tmpWorkingCellList))
			{
				if (!IsOnboardGravship_NewTemp(item, engine, pawn, desperate: true, respectAllowedAreas: false))
				{
					cell = item;
					return true;
				}
