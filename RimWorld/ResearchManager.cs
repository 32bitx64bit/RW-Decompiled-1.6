...
public sealed class ResearchManager : IExposable
{
	...
	private void EnsureKnowledgeProjectsInitialized()
	{
		...
		foreach (KnowledgeCategoryDef i in DefDatabase<KnowledgeCategoryDef>.AllDefs)
		{
			if (!currentAnomalyKnowledgeProjects.Any((KnowledgeCategoryProject x) => x.category == i))
			{
