...
public static class RoomGenUtility
{
	...
	public static void SpawnDormantThreatCluster(LayoutRoom room, Map map, Faction faction, Func<PawnKindDef, bool> selector, float threatPoints, string signalTag = null)
	{
		...
		LordJob_StructureThreatCluster lordJob = new LordJob_StructureThreatCluster(faction, room.rects[0].CenterCell, Mathf.Min(12f, (float)(num * 2)), sendWokenUpMessage: true, awakeOnClamor: true);
	...
	private static List<IntVec3> IdealWallAttatchmentPositions(ThingDef thingDef, Map map, List<CellRect> rects, float thingsPerSide10Cells = 1f, Predicate<IntVec3, Rot4> validator = null)
	{
		...
		foreach (CellRect rect in rects)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				int num2 = Mathf.RoundToInt(num / 3f);
				int num3 = Mathf.Min(Mathf.RoundToInt(num / 10f * thingsPerSide10Cells), num2);
				if (num3 <= 0)
				{
					continue;
				}
				int num4 = Mathf.Max(3, Mathf.RoundToInt(num / (float)(num3 + 1)));
				int num5 = num4;
				for (int j = 0; j < num3; j++)
				{
					for (int k = 0; k < 2; k++)
					{
						...
						for (int l = 0; l < 2; l++)
						{
							...
							IntVec3 centerCellOnEdge = cellRect.GetCenterCellOnEdge(rot2, (k == 0) ? num6 : (-num6));
	...
	{
		int num = Mathf.FloorToInt(((float)rect.Width - 2f) / (float)minRoomSize);
		int num2 = Mathf.FloorToInt(((float)rect.Height - 2f) / (float)minRoomSize);
		int num3 = Rand.Range(0, Mathf.Min(num, maxRooms));
		int num4 = Rand.Range(0, Mathf.Min(num2, maxRooms));
		...
		for (int i = 0; i < num3; i++)
		{
			...
			int num7 = Rand.RangeInclusive(-wallMoveRange, wallMoveRange);
			num5 = item.maxX + num7;
			item.maxX += num7 - 1;
			item2.minX += num7 + 1;
			queue.Enqueue(item);
		...
		for (int j = 0; j < num4; j++)
		{
			...
			for (int k = 0; k < count; k++)
			{
				...
				int num8 = Rand.RangeInclusive(-wallMoveRange, wallMoveRange);
				num6 = item3.maxZ + num8;
				item3.maxZ += num8 - 1;
				item4.minZ += num8 + 1;
				queue.Enqueue(item3);
				queue.Enqueue(item4);
		...
		if (num5 >= 0)
		{
			for (int l = rect.minZ; l <= rect.maxZ; l++)
			{
				...
				GenSpawn.Spawn(ThingMaker.MakeThing(obj, obj.MadeFromStuff ? (wallStuff ?? ThingDefOf.Steel) : null), new IntVec3(num5, 0, l), map);
			}
		}
		if (num6 >= 0)
		{
			for (int m = rect.minX; m <= rect.maxX; m++)
			{
				...
				GenSpawn.Spawn(ThingMaker.MakeThing(obj2, obj2.MadeFromStuff ? (wallStuff ?? ThingDefOf.Steel) : null), new IntVec3(m, 0, num6), map);
			}
		}
		return queue;
	}
