...
public class Building_Bookcase : Building, IThingHolderEvents<Book>, IHaulEnroute, ILoadReferenceable, IStorageGroupMember, IHaulDestination, IStoreSettingsParent, IHaulSource, IThingHolder, ISearchableContents, IBeautyContainer
{
	...
	private static readonly Vector3[] StandardBookshelfRotOffsets = (Vector3[])(object)new Vector3[4]
	{
	...
	protected override void DrawAt(Vector3 drawLoc, bool flip = false)
	{
		...
		Vector3 val = rot.FacingCell.ToVector3() * num;
		Vector3 val2 = rot.FacingCell.ToVector3() * ((float)(-MaximumBooks) * num * 0.5f);
		Vector3 val3 = RotOffsets[base.Rotation.AsInt];
		for (int i = 0; i < HeldBooks.Count; i++)
		{
			...
			Vector3 loc = drawLoc + val2 + val3 + DrawOffset + val * (float)i;
