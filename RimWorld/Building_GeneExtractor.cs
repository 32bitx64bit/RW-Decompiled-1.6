...
public class Building_GeneExtractor : Building_Enterable, IThingHolderWithDrawnPawn, IThingHolder
{
	...
	public Texture2D InsertPawnTex
	{
		get
		{
			if ((Object)(object)cachedInsertPawnTex == (Object)null)
			{
	...
	public override Vector3 PawnDrawOffset => IntVec3.West.RotatedBy(base.Rotation).ToVector3() / (float)def.size.x;
	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		if (base.Working)
		{
			...
			command_Action.icon = (Texture)(object)CancelIcon;
		...
		if (selectedPawn != null)
		{
			...
			command_Action3.icon = (Texture)(object)CancelIcon;
		...
		command_Action4.icon = (Texture)(object)InsertPawnTex;
			...
			}).ThenBy((Pawn p) => p.Label))
			{
				...
				if (pawn.genes != null)
				{
					...
					if (!acceptanceReport.Accepted)
					{
						...
						{
							list.Add(new FloatMenuOption(text + ": " + acceptanceReport.Reason, (Action)null, (Thing)pawn, Color.white, MenuOptionPriority.Default, (Action<Rect>)null, (Thing)null, 0f, (Func<Rect, bool>)null, (WorldObject)null, playSelectionSound: true, 0));
						}
					}
					else
					{
