...
public static class IdeoUIUtility
{
	...
	public static void DoName(ref float curY, float width, Ideo ideo, IdeoEditMode editMode)
	{
		...
		float num = ((editMode != 0) ? 220f : 275f);
		...
		if (structureMeme != null)
		{
			...
			if (editMode != 0 && Widgets.ButtonInvisible(val) && TutorAllowsInteraction(editMode))
			{
		...
		if (ideo.culture != null)
		{
			...
			GUI.color = Color.white;
			if (editMode != 0 && Widgets.ButtonInvisible(val2) && TutorAllowsInteraction(editMode))
			{
	...
	public static void DoStyles(ref float curY, ref float curX, float width, Ideo ideo, IdeoEditMode editMode, int styleIconSize = 28)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (i < tmpStyleCategories.Count)
			{
				...
				{
					list.Add(new FloatMenuOption(s2.LabelCap, delegate
					{
						ideo.thingStyleCategories.RemoveAt(index);
						ideo.thingStyleCategories.Insert(index, new ThingStyleCategoryWithPriority(s2, 3 - index));
						ideo.SortStyleCategories();
						ideo.style.ResetStylesForThingDef();
					}, s2.Icon, Color.white));
				}
			...
			else
			{
				...
				foreach (StyleCategoryDef s in DefDatabase<StyleCategoryDef>.AllDefs.Where((StyleCategoryDef x) => (!x.fixedIdeoOnly || editMode == IdeoEditMode.Dev) && !ideo.thingStyleCategories.Any((ThingStyleCategoryWithPriority y) => y.category == x)))
				{
					...
					{
						ideo.thingStyleCategories.Add(new ThingStyleCategoryWithPriority(s, 3 - index));
						ideo.SortStyleCategories();
						ideo.style.ResetStylesForThingDef();
					}, s.Icon, Color.white));
				}
	...
	private static void DoPreceptsInt(string categoryLabel, string addPreceptLabel, bool mainPrecepts, Ideo ideo, IdeoEditMode editMode, ref float curY, float width, Func<PreceptDef, bool> filter, bool group = false)
	{
		...
		for (int j = 0; j < tmpPrecepts.Count; j++)
		{
			...
			((Rect)(ref val2))._002Ector(num3 + (float)num5 * PreceptBoxSize.x + (float)(num5 * 8), curY + (float)num4 * PreceptBoxSize.y + (float)(num4 * 8), PreceptBoxSize.x, PreceptBoxSize.y);
			if (mainPrecepts && editMode == IdeoEditMode.GameStart)
			{
				UIHighlighter.HighlightOpportunity(val2, "PreceptBox");
			}
			tmpPrecepts[j].DrawPreceptBox(val2, editMode, tmpMouseOverMeme != null && IsPreceptRelatedForUI(tmpMouseOverMeme, tmpPrecepts[j].def));
			if (Mouse.IsOver(val2))
			{
	...
	private static void RegisterPreceptGroup(Ideo ideo, PreceptDef def, IdeoEditMode mode, ThingDef thing = null)
	{
		if (def.issue != null)
		{
			...
			{
				List<FloatMenuOption> list2 = (issueGroupsTmp[def.issue] = new List<FloatMenuOption>());
				value = list2;
	...
	private static void AddPrecept(Ideo ideo, IdeoEditMode editMode, Func<PreceptDef, bool> filter, bool group)
	{
		...
		int num = ideo.PreceptsListForReading.Count((Precept p) => p is Precept_Ritual && (showAll || p.def.visible));
		...
		foreach (PreceptDef p2 in list)
		{
			...
			int num2 = p2.maxCount;
			if (p2.preceptInstanceCountCurve != null)
			{
				num2 = Mathf.Max(num2, Mathf.RoundToInt(p2.preceptInstanceCountCurve.Last().y));
			}
			if (preceptCountOfDef < num2 || p2.ignoreLimitsInEditMode)
			{
				if (!p2.useChoicesFromBuildingDefs || p2.Worker.ThingDefsForIdeo(ideo, null).EnumerableNullOrEmpty())
				{
					...
					{
						RegisterPreceptGroup(ideo, p2, editMode);
					}
					else
					{
				...
				foreach (RitualPatternDef item in DefDatabase<RitualPatternDef>.AllDefs.Where((RitualPatternDef d) => d.patternGroupTag == groupTag && d != p2.ritualPatternBase))
				{
					AddPreceptOption(ideo, p2, editMode, opts, item);
				}
			}
			...
			else if (preceptCountOfDef >= num2 && p2.issue.allowMultiplePrecepts)
			{
				opts.Add(new FloatMenuOption(p2.LabelCap + " (" + "MaxPreceptCount".Translate(num2) + ")", null, p2.Icon ?? ideo.Icon, ideo.Color));
			}
		...
		void AddXenotypePreceptOption(PreceptDef pr)
		{
			...
			foreach (XenotypeDef item3 in DefDatabase<XenotypeDef>.AllDefs.OrderBy((XenotypeDef x) => 0f - x.displayPriority))
			{
				...
				if (!ideo.PreceptsListForReading.Any((Precept y) => y is Precept_Xenotype precept_Xenotype3 && precept_Xenotype3.xenotype == xenotype))
				{
		...
		if (Widgets.ButtonText(new Rect(420f, curY, 200f, 40f), "DEV: Test names..."))
		{
			...
			list2.Add(new FloatMenuOption("Ideo names (replace)", delegate
			{
				...
				for (int l = 0; l < 200; l++)
				{
					ideo.foundation.GenerateTextSymbols();
					ideo.foundation.GenerateLeaderTitle();
					stringBuilder3.AppendLine(ideo.name + "\n- " + ideo.adjective + "\n- " + ideo.memberName + "\n- " + ideo.leaderTitleMale);
