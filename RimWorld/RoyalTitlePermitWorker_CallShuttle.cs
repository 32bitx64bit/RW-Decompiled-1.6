...
public class RoyalTitlePermitWorker_CallShuttle : RoyalTitlePermitWorker_Targeted
{
	...
	public static void DrawShuttleGhost(LocalTargetInfo target, Map map, ThingDef shuttleDef, Rot4 rot)
	{
		...
		Vector3 val = ThingUtility.InteractionCellWhenAt(shuttleDef, target.Cell, rot, map).ToVector3ShiftedWithAltitude(AltitudeLayer.Blueprint);
		Graphics.DrawMesh(MeshPool.plane10, val, Quaternion.identity, GenDraw.InteractionCellMaterial, 0);
	}
