...
public class Dialog_BillConfig : Window
{
	...
	protected override void LateWindowOnGUI(Rect inRect)
	{
		...
		Rect rect = new Rect(((Rect)(ref inRect)).x, ((Rect)(ref inRect)).y, 34f, 34f);
	...
	public override void DoWindowContents(Rect inRect)
	{
		...
		float num = (int)((((Rect)(ref inRect)).width - 34f) / 3f);
		Rect rect = default(Rect);
		((Rect)(ref rect))._002Ector(0f, 80f, num, ((Rect)(ref inRect)).height - 80f);
		Rect rect2 = default(Rect);
		((Rect)(ref rect2))._002Ector(((Rect)(ref rect)).xMax + 17f, 50f, num, ((Rect)(ref inRect)).height - 50f - Window.CloseButSize.y);
		Rect val = default(Rect);
		((Rect)(ref val))._002Ector(((Rect)(ref rect2)).xMax + 17f, 50f, 0f, ((Rect)(ref inRect)).height - 50f - Window.CloseButSize.y);
		((Rect)(ref val)).xMax = ((Rect)(ref inRect)).xMax;
		...
		DoIngredientConfigPane(((Rect)(ref val)).x, ref y, ((Rect)(ref val)).width, ((Rect)(ref val)).height);
		...
		if (bill is Bill_Mech bill_Mech)
		{
			...
			{
				float num2 = (float)(int)bill.recipe.ProducedThingDef.GetStatValueAbstract(StatDefOf.WastepacksPerRecharge) * bill.recipe.ProducedThingDef.GetStatValueAbstract(StatDefOf.BandwidthCost);
				stringBuilder.AppendLine(string.Concat(Find.ActiveLanguageWorker.Pluralize(ThingDefOf.Wastepack.LabelCap) + " " + "ThingsProduced".Translate() + ": ", num2.ToString()));
			}
		}
		...
		if (Text.CalcHeight(text4, ((Rect)(ref rect)).width) > ((Rect)(ref rect)).height)
		{
			Text.Font = GameFont.Tiny;
		}
		...
		{
			listing_Standard5.Gap(((Rect)(ref rect)).height - listing_Standard5.CurHeight - 90f);
			...
			{
				Rect rect3 = listing_Standard5.GetRect(30f);
				Widgets.DrawHighlight(rect3);
				Text.Anchor = (TextAnchor)4;
				Widgets.Label(rect3, "NoStylesAvailable".Translate());
		...
		if (bill.recipe.products.Count == 1)
		{
			...
			Widgets.InfoCardButton(num3, ((Rect)(ref val)).y, thingDef3, GenStuff.DefaultStuffFor(thingDef3));
			num3 += 28f;
		}
		if (Widgets.ButtonImage(new Rect(num3, ((Rect)(ref val)).y, 30f, 30f), TexButton.Rename))
		{
	...
	protected virtual void DoIngredientConfigPane(float x, ref float y, float width, float height)
	{
		...
		{
			Rect rect = default(Rect);
			((Rect)(ref rect))._002Ector(x, y, width, height - (float)IngredientRadiusSubdialogHeight);
		...
		Rect rect2 = default(Rect);
		((Rect)(ref rect2))._002Ector(x, y, width, (float)IngredientRadiusSubdialogHeight);
		Listing_Standard listing_Standard = new Listing_Standard();
		listing_Standard.Begin(rect2);
		string text = "IngredientSearchRadius".Translate().Truncate(((Rect)(ref rect2)).width * 0.6f);
		string text2 = ((bill.ingredientSearchRadius == 999f) ? "Unlimited".TranslateSimple().Truncate(((Rect)(ref rect2)).width * 0.3f) : bill.ingredientSearchRadius.ToString("F0"));
