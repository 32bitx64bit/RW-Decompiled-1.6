...
public static class SpectatorCellFinder
{
	...
	public static void DebugFlashPotentialSpectatorCells(CellRect spectateRect, Map map, SpectateRectSide allowedSides = SpectateRectSide.All, int margin = 1)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			list.Add(cell);
			float num2 = Mathf.Lerp(1f, 0.08f, (float)i / (float)num);
			Material mat = SolidColorMaterials.SimpleSolidColorMaterial(new Color(0f, 0.8f, 0f, num2));
	...
	public static Vector3 GraphicOffsetForRect(this SpectateRectSide side, IntVec3 center, CellRect rect, Rot4 rotation, Vector2 additionalOffset)
	{
		...
		Vector2 val = asVector;
		((Vector2)(ref val)).Scale(Vector2.op_Implicit(new Vector3((float)rect.Width / 2f, (float)rect.Height / 2f)));
		Vector2 val2 = default(Vector2);
		((Vector2)(ref val2))._002Ector((rect.Width % 2 == 0) ? 0.5f : 0f, (rect.Height % 2 == 0) ? 0.5f : 0f);
		if (rotation.AsInt > 1)
		{
			val2 = -val2;
		}
		...
		val2 += (((spectateRectSide & SpectateRectSide.Horizontal) != SpectateRectSide.Horizontal) ? asVector : (-asVector)) * additionalOffset;
		val2 += new Vector2(((rect.Width % 2 != 0) ? 0.75f : 0.5f) * asVector.x, ((rect.Height % 2 != 0) ? 0.75f : 0.5f) * asVector.y);
		return center.ToVector3Shifted() + new Vector3(val.x, 0f, val.y) + new Vector3(val2.x, 0f, val2.y);
	}
