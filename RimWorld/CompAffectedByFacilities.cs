...
public class CompAffectedByFacilities : ThingComp
{
	...
	public static bool CanPotentiallyLinkTo_Static(ThingDef facilityDef, IntVec3 facilityPos, Rot4 facilityRot, ThingDef myDef, IntVec3 myPos, Rot4 myRot, Map myMap)
	{
		...
		{
			Vector3 val = GenThing.TrueCenter(myPos, myRot, myDef.size, myDef.Altitude);
			Vector3 val2 = GenThing.TrueCenter(facilityPos, facilityRot, facilityDef.size, facilityDef.Altitude);
	...
	private bool IsBetter(ThingDef facilityDef, IntVec3 facilityPos, Rot4 facilityRot, Thing thanThisFacility)
	{
		...
		Vector3 val = GenThing.TrueCenter(facilityPos, facilityRot, facilityDef.size, facilityDef.Altitude);
		Vector3 val2 = parent.TrueCenter();
		float num = Vector3.Distance(val2, val);
		float num2 = Vector3.Distance(val2, thanThisFacility.TrueCenter());
		if (num != num2)
		{
			return num < num2;
		}
