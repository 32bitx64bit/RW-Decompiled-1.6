...
public class RoomPart_Gore : RoomPartWorker
{
	...
	private void SpawnCorpseSmear(Map map, IntVec3 cell, LayoutRoom room, Faction faction)
	{
		...
		while (num > 0 && ValidCell(map, intVec, room))
		{
			...
			if (outFilth2 != null)
			{
				Vector3 val = (intVec2 - intVec).ToVector3();
				Vector3 v = intVec.ToVector3() + val * 0.5f;
				outFilth2.SetOverrideDrawPositionAndRotation(v.WithY(ThingDefOf.Filth_BloodSmear.Altitude), val.AngleFlat());
