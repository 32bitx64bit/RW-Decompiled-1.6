...
public class MeditationUtility
{
	...
	public static MeditationSpotAndFocus FindMeditationSpot(Pawn pawn)
	{
		...
		foreach (LocalTargetInfo item in AllMeditationSpotCandidates(pawn))
		{
			...
			float num2 = 1f / Mathf.Max((float)item.Cell.DistanceToSquared(pawn.Position), 0.1f);
	...
	public static IEnumerable<LocalTargetInfo> AllMeditationSpotCandidates(Pawn pawn, bool allowFallbackSpots = true)
	{
		...
		foreach (Thing item2 in list)
		{
			...
			if ((room2 == null || CanUseRoomToMeditate(room2, pawn)) && item2.GetStatValueForPawn(StatDefOf.MeditationFocusStrength, pawn) > 0f)
			{
		...
		{
			foreach (LocalTargetInfo item3 in FocusSpotsInTheRoom(pawn, room3))
			{
				yield return item3;
			}
		...
		{
			foreach (LocalTargetInfo item4 in FocusSpotsInTheRoom(pawn, room))
			{
			...
			intVec = RCellFinder.RandomWanderDestFor(pawn, randomCell, WanderRadius, (Pawn p, IntVec3 c, IntVec3 r) => c.GetRoom(p.Map) == room && c.Standable(p.Map) && c.GetDoor(p.Map) == null && WanderRoomUtility.IsValidWanderDest(p, c, r), pawn.NormalMaxDanger());
			if (intVec.IsValid)
			{
				yield return intVec;
			}
