...
public class Bombardment : OrbitalStrike
{
	public class BombardmentProjectile : IExposable
	{
		...
		public void Draw(Material material)
		{
			...
			if (lifeTime > 0)
			{
				Vector3 val = targetCell.ToVector3() + Vector3.forward * Mathf.Lerp(60f, 0f, 1f - (float)lifeTime / (float)maxLifeTime);
				val.z += 1.25f;
				val.y = AltitudeLayer.MoteOverhead.AltitudeFor();
				Matrix4x4 val2 = Matrix4x4.TRS(val, Quaternion.Euler(0f, 180f, 0f), new Vector3(2.5f, 1f, 2.5f));
				Graphics.DrawMesh(MeshPool.plane10, val2, material, 0);
			}
		}
