...
public sealed class PlaySettings : IExposable
{
	...
	private void DoWorldViewControls(WidgetRow row)
	{
		...
		if (row.ButtonIcon(TexButton.SearchButton, tooltip) || (KeyBindingDefOf.OpenMapSearch.JustPressed && (int)Event.current.type == 4))
		{
	...
	private void DoMapControls(WidgetRow row)
	{
		...
		if (row.ButtonIcon(TexButton.SearchButton, tooltip3) || (KeyBindingDefOf.OpenMapSearch.JustPressed && (int)Event.current.type == 4))
		{
			Event.current.Use();
