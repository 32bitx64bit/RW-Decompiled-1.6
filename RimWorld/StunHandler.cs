	...
	public void Notify_DamageApplied(DamageInfo dinfo)
	{
		...
		if (CanAdaptToDamage(dinfo.Def))
		{
			...
			else
			{
				...
				{
					MoteMaker.ThrowText(new Vector3((float)parent.Position.x + 1f, (float)parent.Position.y, (float)parent.Position.z + 1f), text: dinfo.Def.adaptedText ?? ((string)"Adapted".Translate()), map: parent.Map, color: Color.white);
				}
				num = 0f;
			}
		}
