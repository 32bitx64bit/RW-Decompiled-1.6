		...
		for (int i = 0; i < tmpThings.Count; i++)
		{
			...
			if (TryFindClosestReachableNet(compPowerBattery.parent.Position, (PowerNet x) => HasAnyPowerGenerator(x), map, out var foundNet, out var closestTransmitter))
			{
				...
				if (canSpawnPowerGenerators)
				{
					...
					if (Rand.Chance(Mathf.InverseLerp((float)MaxDistanceBetweenBatteryAndTransmitter.min, (float)MaxDistanceBetweenBatteryAndTransmitter.max, (float)count)) && TrySpawnPowerGeneratorNear(compPowerBattery.parent.Position, map, compPowerBattery.parent.Faction, out var newPowerGenerator))
					{
