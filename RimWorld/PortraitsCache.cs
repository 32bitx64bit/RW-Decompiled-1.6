...
public static class PortraitsCache
{
	...
	private readonly struct PortraitParams : IEquatable<PortraitParams>
	{
		...
		public PortraitParams(Vector2 size, Vector3 cameraOffset, float cameraZoom, Rot4 rotation, bool renderHeadgear = true, bool renderClothes = true, IReadOnlyDictionary<Apparel, Color> overrideApparelColors = null, Color? overrideHairColor = null, bool stylingStation = false, PawnHealthState? overrideHealthState = null)
		{
			...
			int num = ((object)(Vector2)(ref size)).GetHashCode() ^ ((object)(Vector3)(ref cameraOffset)).GetHashCode() ^ cameraZoom.GetHashCode() ^ rotation.GetHashCode() ^ renderHeadgear.GetHashCode() ^ renderClothes.GetHashCode() ^ stylingStation.GetHashCode();
			int num2;
			...
			{
				Color valueOrDefault = overrideHairColor.GetValueOrDefault();
				num2 = ((object)(Color)(ref valueOrDefault)).GetHashCode();
			}
			cachedHashCode = num ^ num2 ^ (overrideHealthState?.GetHashCode() ?? 0) ^ GenCollection.DictHashCode(overrideApparelColors);
		}
		...
		public bool Equals(PortraitParams other)
		{
			...
			{
				Color? val = other.overrideHairColor;
				Color? val2 = overrideHairColor;
				if (val.HasValue == val2.HasValue && (!val.HasValue || val.GetValueOrDefault() == val2.GetValueOrDefault()) && other.overrideHealthState == overrideHealthState)
				{
	...
	public static RenderTexture Get(Pawn pawn, Vector2 size, Rot4 rotation, Vector3 cameraOffset = default(Vector3), float cameraZoom = 1f, bool supersample = true, bool compensateForUIScale = true, bool renderHeadgear = true, bool renderClothes = true, IReadOnlyDictionary<Apparel, Color> overrideApparelColors = null, Color? overrideHairColor = null, bool stylingStation = false, PawnHealthState? healthStateOverride = null)
	{
		...
		RenderTexture val = NewRenderTexture(size);
		portraitParams.RenderPortrait(pawn, val);
		orCreateCachedPortraitsWithParams.Add(pawn, new CachedPortrait(val, dirty: false, Time.time));
		return val;
	}
	...
	private static RenderTexture NewRenderTexture(Vector2 size)
	{
		...
		int num = renderTexturesPool.FindLastIndex((RenderTexture x) => ((Texture)x).width == (int)size.x && ((Texture)x).height == (int)size.y);
		if (num != -1)
		{
