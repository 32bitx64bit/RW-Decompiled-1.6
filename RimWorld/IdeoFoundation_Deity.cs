...
public class IdeoFoundation_Deity : IdeoFoundation
{
	...
	public override void DoInfo(ref float curY, float width, IdeoEditMode editMode)
	{
		...
		if (editMode != 0)
		{
			...
			((Rect)(ref val))._002Ector(num - IdeoUIUtility.ButtonSize.x, curY - IdeoUIUtility.ButtonSize.y, IdeoUIUtility.ButtonSize.x, IdeoUIUtility.ButtonSize.y);
			Rect val2 = val;
			((Rect)(ref val2)).x = ((Rect)(ref val)).xMin - ((Rect)(ref val)).width - 10f;
			bool num2 = deities.Count < ideo.DeityCountRange.max;
			if (Widgets.ButtonText(num2 ? val2 : val, "RandomizeDeities".Translate()) && IdeoUIUtility.TutorAllowsInteraction(editMode))
			{
				GenerateDeities();
			...
			if (num2 && Widgets.ButtonText(val, "AddDeity".Translate()) && IdeoUIUtility.TutorAllowsInteraction(editMode))
			{
		...
		for (int i = 0; i < deities.Count; i++)
		{
			...
			((Rect)(ref val3))._002Ector(num6 + (float)num4 * DeityBoxSize.x + (float)(num4 * 8), curY + (float)num3 * DeityBoxSize.y + (float)(num3 * 8), DeityBoxSize.x, DeityBoxSize.y);
			Widgets.DrawLightHighlight(val3);
			if (Mouse.IsOver(val3))
			{
				...
				TooltipHandler.TipRegion(val3, text);
			}
			if (Widgets.ButtonInvisible(val3) && editMode != 0 && IdeoUIUtility.TutorAllowsInteraction(editMode))
			{
			...
			((Rect)(ref val4))._002Ector(((Rect)(ref val3)).x + (((Rect)(ref val3)).height - 50f) / 2f, ((Rect)(ref val3)).y + (((Rect)(ref val3)).height - 50f) / 2f, 50f, 50f);
			GUI.DrawTexture(val4, (Texture)(object)deities[i].Icon);
			Rect rect = new Rect(((Rect)(ref val4)).xMax + 10f, ((Rect)(ref val3)).y + 3f, ((Rect)(ref val3)).xMax - ((Rect)(ref val4)).xMax - 10f, ((Rect)(ref val3)).height / 2f);
			Text.Anchor = (TextAnchor)3;
			Widgets.Label(rect, deities[i].name);
			GenUI.ResetLabelAlign();
			Rect rect2 = new Rect(((Rect)(ref val4)).xMax + 10f, ((Rect)(ref val3)).y + ((Rect)(ref val3)).height / 2f - 3f, ((Rect)(ref val3)).xMax - ((Rect)(ref val4)).xMax - 10f, ((Rect)(ref val3)).height / 2f);
			Text.Anchor = (TextAnchor)3;
			GUI.color = new Color(0.8f, 0.8f, 0.8f);
			Widgets.Label(rect2, deities[i].type);
			GUI.color = Color.white;
			GenUI.ResetLabelAlign();
			GUI.DrawTexture(new Rect(((Rect)(ref val3)).xMax - 20f - 4f, ((Rect)(ref val3)).y + 4f, 20f, 20f), (Texture)(object)deities[i].gender.GetIcon());
		}
	...
	{
		deities.Clear();
		int num = ((!Rand.Chance(0.5f)) ? (Rand.Chance(0.5f) ? 1 : Rand.RangeInclusive(2, 4)) : 0);
		...
		IEnumerable<string> AllExistingDeities()
		{
			...
			if (Find.World != null)
			{
				...
				for (int i = 0; i < ideos.Count; i++)
				{
					...
					{
						for (int k = 0; k < deityFoundation.deities.Count; k++)
						{
							yield return deityFoundation.deities[k].name;
						}
					}
				}
			}
