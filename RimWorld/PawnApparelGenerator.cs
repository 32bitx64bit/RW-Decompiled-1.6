...
public static class PawnApparelGenerator
{
	private class PossibleApparelSet
	{
		...
		public void AddFreeVacuumResistanceAsNeeded(Pawn pawn)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				if (!allApparelPairs.Where(VacuumValidator).TryRandomElementByWeight((ThingStuffPair pa) => pa.Commonality / (pa.Price * pa.Price), out var pair2))
				{
					continue;
				}
				if (DebugViewSettings.logApparelGeneration)
				{
					debugSb.AppendLine($"Giving free vacuum resistance: {pair2} VacuumResistance={pair2.VacuumResistance}");
					foreach (ThingStuffPair item in aps.Where((ThingStuffPair a) => !ApparelUtility.CanWearTogether(a.thing, pair2.thing, body)))
					{
				...
				for (int num = aps.Count - 1; num >= 0; num--)
				{
					...
					if (!ApparelUtility.CanWearTogether(thingStuffPair.thing, pair2.thing, body))
					{
						if (DebugViewSettings.logApparelGeneration)
						{
				...
				aps.Add(pair2);
				desired -= pair2.VacuumResistance;
			}
			if (DebugViewSettings.logApparelGeneration)
			{
			...
			bool VacuumValidator(ThingStuffPair pair)
			{
				if (!pair.thing.apparel.canBeGeneratedToSatisfyVacuumResistance)
				{
					return false;
				}
				if (pair.VacuumResistance < 0.1f)
				{
					return false;
				}
				if (pair.Price > freeVacuumResistanceApparelMaxPrice)
				{
					return false;
				}
				if (!pair.thing.apparel.CorrectAgeForWearing(pawn))
				{
					return false;
				}
				if (!CoversUnprotectedVacuumPart(pair.thing))
				{
					return false;
				}
				for (int j = 0; j < aps.Count; j++)
				{
					if (!ApparelUtility.CanWearTogether(aps[j].thing, pair.thing, body) && pair.VacuumResistance - aps[j].VacuumResistance <= 0.15f)
					{
						return false;
					}
				}
	...
	public static void GenerateStartingApparelFor(Pawn pawn, PawnGenerationRequest request)
	{
		...
		for (int i = 0; i < allApparelPairs.Count; i++)
		{
			...
			if (CanUsePair(thingStuffPair, pawn, randomInRange, flag2, @int))
			{
	...
	private static bool NeedVacuumResistance(Pawn pawn, PawnGenerationRequest request)
	{
		...
		if (!request.Tile.Valid || !request.Tile.LayerDef.isSpace)
		{
			return false;
		}
