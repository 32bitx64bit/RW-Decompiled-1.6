...
public class GameComponent_PawnDuplicator : GameComponent
{
	...
	private static void CopyGenes(Pawn pawn, Pawn newPawn)
	{
		if (ModsConfig.BiotechActive)
		{
			...
			int j;
			for (j = 0; j < sourceXenogenes.Count; j++)
			{
				Gene gene = newPawn.genes.Xenogenes[j];
				if (sourceXenogenes[j].Overridden)
				{
					gene.overriddenByGene = newPawn.genes.GenesListForReading.First((Gene e) => e.def == sourceXenogenes[j].overriddenByGene.def);
				}
				else
				{
		...
		int i;
		for (i = 0; i < sourceEndogenes.Count; i++)
		{
			Gene gene2 = newPawn.genes.Endogenes[i];
			if (sourceEndogenes[i].Overridden)
			{
				gene2.overriddenByGene = newPawn.genes.GenesListForReading.First((Gene e) => e.def == sourceEndogenes[i].overriddenByGene.def);
			}
			else
			{
