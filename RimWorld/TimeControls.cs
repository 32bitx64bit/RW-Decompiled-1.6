...
public static class TimeControls
{
	...
	public static void DoTimeControlsGUI(Rect timerRect)
	{
		...
		((Rect)(ref val))._002Ector(0f, 0f, TimeButSize.x, TimeButSize.y);
		for (int i = 0; i < CachedTimeSpeedValues.Length; i++)
		{
			...
			if (Widgets.ButtonImage(val, TexButton.SpeedButtonTextures[(uint)timeSpeed], doMouseoverSound: true, tooltip) && !tickManager.ForcePaused)
			{
		...
		{
			Widgets.DrawLineHorizontal(((Rect)(ref val)).width * 2f, ((Rect)(ref val)).height / 2f, ((Rect)(ref val)).width * 2f);
		}
		if (tickManager.ForcePaused)
		{
			Widgets.DrawLineHorizontal(((Rect)(ref val)).width, ((Rect)(ref val)).height / 2f, ((Rect)(ref val)).width * 3f);
		}
		Widgets.EndGroup();
