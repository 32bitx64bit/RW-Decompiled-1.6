...
public sealed class GameConditionManager : IExposable
{
	...
	public void DoConditionsUI(Rect rect)
	{
		...
		for (int i = 0; i < activeConditions.Count; i++)
		{
			...
			((Rect)(ref val))._002Ector(0f, num, ((Rect)(ref rect)).width, Text.CalcHeight(labelCap, ((Rect)(ref rect)).width - 6f));
			Text.Font = GameFont.Small;
			Text.Anchor = (TextAnchor)5;
			Widgets.DrawHighlightIfMouseover(val);
			Rect rect2 = val;
			((Rect)(ref rect2)).width = ((Rect)(ref rect2)).width - 6f;
			Widgets.Label(rect2, labelCap);
			if (Mouse.IsOver(val))
			{
				TooltipHandler.TipRegion(val, new TipSignal(activeConditions[i].TooltipString, 0x3A2DF42A ^ i));
			}
