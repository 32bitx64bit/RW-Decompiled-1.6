...
public class Ability : IVerbOwner, IExposable, ILoadReferenceable
{
	...
	private void MoteTick()
	{
		...
		if ((def.warmupMote == null && (Object)(object)def.WarmupMoteSocialSymbol == (Object)null) || !Casting)
		{
			return;
		}
		Vector3 val = pawn.DrawPos + def.moteDrawOffset;
		val += (verb.CurrentTarget.CenterVector3 - val) * def.moteOffsetAmountTowardsTarget;
		if (warmupMote == null || warmupMote.Destroyed)
		{
			if ((Object)(object)def.WarmupMoteSocialSymbol == (Object)null)
			{
				if (def.warmupMote.thingClass != typeof(MoteAttached))
				{
					warmupMote = MoteMaker.MakeStaticMote(val, pawn.Map, def.warmupMote);
				}
				else
				{
	...
	private void SetupWarmupEffecter(LocalTargetInfo target)
	{
		...
		else
		{
			...
			{
				val += item.Cell.ToVector3Shifted();
			}
			val /= (float)affectedTargets.Count();
			warmupEffecter = def.warmupEffecter.Spawn(val.ToIntVec3(), pawn.MapHeld);
			verbTargetInfoTmp = new TargetInfo(val.ToIntVec3(), pawn.MapHeld);
		}
