...
public class Building_GrowthVat : Building_Enterable, IStoreSettingsParent, IThingHolderWithDrawnPawn, IThingHolder
{
	...
	public override void SpawnSetup(Map map, bool respawningAfterLoad)
	{
		...
		if (respawningAfterLoad && selectedEmbryo != null && innerContainer.Contains(selectedEmbryo))
		{
			LongEventHandler.ExecuteWhenFinished(delegate
			{
				...
				Color val = EmbryoColor();
				fetusEarlyStageGraphic = FetusEarlyStage.GetColoredVersion(ShaderDatabase.Cutout, val, val);
				fetusLateStageGraphic = FetusLateStage.GetColoredVersion(ShaderDatabase.Cutout, val, val);
			});
		}
	}
	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		if (base.Working)
		{
			...
			command_Action.icon = (Texture)(object)CancelLoadingIcon;
			...
			if (selectedEmbryo != null)
			{
				yield return new Command_Action
				{
					...
					icon = (Texture)(object)GeneSetHolderBase.GeneticInfoTex.Texture,
					action = delegate
					{
		...
		else
		{
			if (selectedPawn != null || selectedEmbryo != null)
			{
				...
				command_Action2.icon = (Texture)(object)CancelLoadingIcon;
			...
			if (selectedPawn == null)
			{
				...
				command_Action3.icon = (Texture)(object)InsertPawnIcon.Texture;
				command_Action3.action = delegate
				{
					...
					foreach (Pawn item2 in base.Map.mapPawns.AllPawnsSpawned)
					{
						...
						{
							list2.Add(new FloatMenuOption(item2.LabelCap, (Action)delegate
							{
								SelectPawn(pawn);
			...
			if (selectedEmbryo == null && Find.Storyteller.difficulty.ChildrenAllowed)
			{
				...
				command_Action4.icon = (Texture)(object)InsertEmbryoIcon.Texture;
