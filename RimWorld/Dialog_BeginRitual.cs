...
public class Dialog_BeginRitual : Dialog_BeginLordJob
{
	...
	public void DrawExtraRitualOutcomeDescriptions(Rect viewRect, float totalQuality, ref float curY, ref float totalInfoHeight)
	{
		...
		for (int i = 0; i < outcome.extraOutcomeDescriptions?.Count; i++)
		{
			...
			if (val.x > ((Rect)(ref viewRect)).width)
			{
				((Vector2)(ref val))._002Ector(((Rect)(ref viewRect)).width, Text.CalcHeight(taggedString, ((Rect)(ref viewRect)).width));
			}
			Widgets.Label(new Rect(((Rect)(ref viewRect)).x, curY, val.x, Mathf.Max(32f, val.y)), taggedString);
			curY += val.y;
			totalInfoHeight += val.y;
		}
	}
	...
	public void DrawExtraInfo(Rect viewRect, ref float curY, ref float totalInfoHeight)
	{
		...
		{
			float num = Math.Max(Text.CalcHeight(extraInfo, ((Rect)(ref viewRect)).width) + 3f, 28f);
			Widgets.Label(new Rect(((Rect)(ref viewRect)).x, curY + 4f, ((Rect)(ref viewRect)).width, num), extraInfo);
			curY += num;
			totalInfoHeight += num;
		}
	}
	...
	public void DrawOutcomeWarnings(Rect viewRect, ref float curY, ref float totalInfoHeight)
	{
		...
		{
			float num = Math.Max(Text.CalcHeight(sleepingWarning, ((Rect)(ref viewRect)).width) + 3f, 28f);
			Widgets.Label(new Rect(((Rect)(ref viewRect)).x, curY + 4f, ((Rect)(ref viewRect)).width, num), sleepingWarning);
			curY += num;
		}
	}
	...
	public void DrawFinalOutcomeInfo(Rect viewRect, ref float curY, ref float totalInfoHeight)
	{
		...
		if (developmentPointsOverOutcomeIndexCurveForRitual != null)
		{
			...
			Widgets.Label(new Rect(((Rect)(ref viewRect)).x, curY, ((Rect)(ref viewRect)).width, num), taggedString);
			curY += num;
			for (int i = 0; i < outcome.outcomeChances.Count; i++)
			{
				...
				Widgets.Label(new Rect(((Rect)(ref viewRect)).x, curY, ((Rect)(ref viewRect)).width, 32f), label);
				curY += Text.LineHeight;
				totalInfoHeight += Text.LineHeight;
			}
		}
	...
	public override void DoExtraHeaderInfo(ref RectDivider layout, ref RectDivider headerLabelRow)
	{
		...
		if (ModsConfig.IdeologyActive && ritual.ideo != null && !Find.IdeoManager.classicMode && !ritual.def.isNonIdeoRitual)
		{
			...
			Vector2 val = Text.CalcSize(ideo.name.CapitalizeFirst());
			IdeoUIUtility.DrawIdeoPlate(headerLabelRow.NewRow(val.y).NewCol(val.x + 30f, HorizontalJustification.Right), ideo);
		}
		using (new TextBlock(GameFont.Small, (TextAnchor)8, false, Color.gray))
		{
			if (!ritual.Label.EqualsIgnoreCase(ritual.UIInfoFirstLine))
			{
				Rect rect = default(Rect);
				((Rect)(ref rect))._002Ector((Rect)headerLabelRow);
				string uIInfoFirstLine = ritual.UIInfoFirstLine;
				Rect rect2 = headerLabelRow.Rect;
				((Rect)(ref rect)).height = Text.CalcHeight(uIInfoFirstLine, ((Rect)(ref rect2)).width);
	...
	protected override void DrawQualityFactors(Rect viewRect)
	{
		...
		if (precept_Ritual != null && precept_Ritual.outcomeEffect?.ShowQuality == false)
		{
			...
			string text = GenCollection.MaxBy(ritual.outcomeEffect.def.outcomeChances, (RitualOutcomePossibility c) => c.positivityIndex).label.CapitalizeFirst();
			TaggedString taggedString = "RitualOutcomeNoQuality".Translate() + ":\n\n  - " + text + " " + ritual.Label;
			Rect rect = val;
			((Rect)(ref rect)).width = ((Rect)(ref rect)).width + 10f;
			((Rect)(ref rect)).height = Text.CalcHeight(taggedString, ((Rect)(ref val)).width);
	...
	private void DrawRoleSelection(Pawn pawn, Rect rect)
	{
		...
		((Rect)(ref rect2))._002Ector(((Rect)(ref rect)).x + 220f, ((Rect)(ref rect)).y + 2f, 140f, 32f);
	...
	public void DrawTargetLocation(Rect targetRow)
	{
		...
		Rect rect = default(Rect);
		((Rect)(ref rect))._002Ector(((Rect)(ref targetRow)).xMax - (x2 + 4f + 32f), ((Rect)(ref targetRow)).y - 6f, 32f, 32f);
		Rect rect2 = default(Rect);
		((Rect)(ref rect2))._002Ector(((Rect)(ref targetRow)).xMax - num, ((Rect)(ref targetRow)).y, x, 32f);
		Rect rect3 = default(Rect);
		((Rect)(ref rect3))._002Ector(((Rect)(ref targetRow)).xMax - (x2 + 4f + 32f) + 32f + 4f, ((Rect)(ref targetRow)).y, x2, 32f);
