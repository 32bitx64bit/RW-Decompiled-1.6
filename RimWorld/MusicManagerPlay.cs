...
public class MusicManagerPlay
{
	...
	public void MusicUpdate()
	{
		...
		float num = Mathf.Clamp01((Time.time - pauseToggledTime) / 0.5f);
		pausedVolumeFactor = Mathf.Clamp01(Mathf.Lerp(pausedVolumeFactorAtToggle, (float)((!isPaused) ? 1 : 0), num));
		if (songWasForced)
		{
		...
		{
			float num2 = (DangerMusicMode ? SongIntervalTension.RandomInRange : SongIntervalRelax.RandomInRange);
	...
	private float GetFadeFactor(Vector3 pos, FloatRange soundRange, Camera camera)
	{
		...
		Vector3 val = ((Component)camera).transform.position - pos;
		val.y = Mathf.Max(val.y - 15f, 0f);
		val.y *= 3.5f;
		return Mathf.Min(Mathf.Max(((Vector3)(ref val)).magnitude - soundRange.min, 0f) / (soundRange.max - soundRange.min), 1f);
	}
	...
	private void InitializeMusicManager()
	{
		...
		val.transform.parent = Find.Root.soundRoot.sourcePool.sourcePoolCamera.cameraSourcesContainer.transform;
