...
public class JobDriver_Dance : JobDriver
{
	...
	public override Vector3 ForcedBodyOffset
	{
		get
		{
			...
			if (jumping)
			{
				float num2 = Mathf.Max(Mathf.Pow((num + 1f) * 0.5f, 2f) * 0.2f - 0.06f, 0f);
				return new Vector3(0f, 0f, num2);
			}
			float num3 = Mathf.Sign(num);
			return new Vector3(EasingFunctions.EaseInOutQuad(Mathf.Abs(num) * 0.6f) * 0.09f * num3, 0f, 0f);
		}
	}
