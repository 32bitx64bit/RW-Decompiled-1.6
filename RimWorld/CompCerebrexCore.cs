...
public class CompCerebrexCore : CompInteractable
{
	...
	private Graphic BrainGraphic => cachedBrainGraphic ?? (cachedBrainGraphic = GraphicDatabase.Get<Graphic_Multi>("Things/Building/CerebrexCore/CerebrexCore_Brain", ShaderDatabase.Cutout, Vector2.op_Implicit(BrainDrawSize), Color.white));
	...
	public override void DrawAt(Vector3 drawLoc, bool flip = false)
	{
		...
		float num = ZOffset + 0.5f * (1f + Mathf.Sin(MathF.PI * 2f * (float)GenTicks.TicksGame / 300f)) * BobHeight;
		Matrix4x4 matrix = Matrix4x4.TRS(drawLoc + new Vector3(0f, 0.35f, num), Quaternion.AngleAxis(0f, Vector3.up), Vector3.one);
