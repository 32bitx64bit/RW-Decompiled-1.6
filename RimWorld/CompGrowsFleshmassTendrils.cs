...
public class CompGrowsFleshmassTendrils : ThingComp
{
	...
	protected virtual void Grow()
	{
		...
		if (!(intVec == IntVec3.Invalid))
		{
			...
			furthestGrownSquared = Mathf.Max(furthestGrownSquared, (float)intVec.DistanceToSquared(Position));
	...
	private bool CanPlaceFleshmass(IntVec3 cell)
	{
		...
		{
			if (!(thing is Building_Door) && !(thing is Building { IsClearableFreeBuilding: not false }) && !(thing is Blueprint) && !(thing.def?.building?.isPowerConduit).GetValueOrDefault() && !(thing.def?.building?.isAttachment).GetValueOrDefault())
			{
