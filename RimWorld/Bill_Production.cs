...
public class Bill_Production : Bill, IRenameable
{
	...
	protected override void DoConfigInterface(Rect baseRect, Color baseColor)
	{
		...
		GUI.color = baseColor;
		WidgetRow widgetRow = new WidgetRow(((Rect)(ref baseRect)).xMax, ((Rect)(ref baseRect)).y + 29f, UIDirection.LeftThenUp);
	...
	{
		if (paused && new WidgetRow(((Rect)(ref rect)).xMax, ((Rect)(ref rect)).y, UIDirection.LeftThenUp).ButtonText("Unpause".Translate()))
		{
			paused = false;
		}
	}
