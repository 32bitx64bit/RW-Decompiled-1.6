...
public abstract class Building_Trap : Building, IPathFindCostProvider
{
	...
	public bool KnowsOfTrap(Pawn p)
	{
		...
		if (p.lord?.LordJob is ILordAvoidTraps lordAvoidTraps && Rand.ValueAsync(HashCode.Combine<int, int>(p.Faction?.loadID ?? p.lord.loadID, thingIDNumber)) < lordAvoidTraps.AvoidTrapRatio)
		{
			return true;
		}
		return false;
	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		if (CanSetAutoRearm)
		{
			...
			command_Toggle.icon = (Texture)(object)TexCommand.RearmTrap;
