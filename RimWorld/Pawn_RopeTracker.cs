...
public class Pawn_RopeTracker : IExposable
{
	...
	public void RopingDraw()
	{
		...
		if (ropedTo.IsValid)
		{
			Vector3 val = ropedTo.CenterVector3.Yto0();
			if (!ropedTo.HasThing && ropedTo.Cell.GetFirstThing(pawn.Map, ThingDefOf.CaravanPackingSpot) != null)
			{
				val += CaravanHitchingPostOffset;
			}
			GenDraw.DrawLineBetween(pawn.DrawPos.Yto0(), val, AltitudeLayer.PawnRope.AltitudeFor(), RopeLineMat);
		}
	}
