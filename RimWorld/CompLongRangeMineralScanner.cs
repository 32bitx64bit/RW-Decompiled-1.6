...
public class CompLongRangeMineralScanner : CompScanner
{
	...
	public override IEnumerable<Gizmo> CompGetGizmosExtra()
	{
		...
		command_Action.icon = (Texture)(object)mineableThing.uiIcon;
		...
		command_Action.action = delegate
		{
			...
			foreach (ThingDef item3 in mineables)
			{
				...
				}, MenuOptionPriority.Default, null, null, 29f, (Rect rect) => Widgets.InfoCardButton(((Rect)(ref rect)).x + 5f, ((Rect)(ref rect)).y + (((Rect)(ref rect)).height - 24f) / 2f, localD.building.mineableThing));
				list.Add(item);
			}
