...
public class Frame : Building, IThingHolder, IConstructible, IStorageGroupMember, IStoreSettingsParent, IHaulEnroute, ILoadReferenceable
{
	...
	protected override void DrawAt(Vector3 drawLoc, bool flip = false)
	{
		...
		Vector3 val = drawLoc;
		if ((BuildDef?.building?.isAttachment).GetValueOrDefault())
		{
		...
		for (int i = 0; i < num; i++)
		{
			...
			val5.x = (float)intVec.x * ((float)base.RotatedSize.x / 2f - num2 / 2f);
			val5.z = (float)intVec.z * ((float)base.RotatedSize.z / 2f - num2 / 2f);
