...
public class GenStep_Asteroid : GenStep
{
	...
	private void SpawnOres(Map map, GenStepParams parms)
	{
		...
		if (num == 0)
		{
			Debug.LogError((object)("No count found for resource " + thingDef));
			return;
		}
	...
	private static void GenerateRuins(Map map, GenStepParams parms)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			Vector3 vect = Vector3Utility.RotatedBy(new Vector3((float)Mathf.RoundToInt((float)map.Size.x * num), 0f, 0f), Rand.Range(0f, 360f));
