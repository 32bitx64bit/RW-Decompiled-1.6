...
public class Building_Bed : Building
{
	...
	private static readonly Color SheetColorForSlave = Color32.op_Implicit(new Color32((byte)252, (byte)244, (byte)3, byte.MaxValue));

	private static readonly Color SheetColorMedicalForSlave = Color32.op_Implicit(new Color32((byte)153, (byte)148, (byte)0, byte.MaxValue));
	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		if (def.building.bed_humanlike && !ForHumanBabies)
		{
			...
			else
			{
				...
				command_Toggle.icon = (Texture)(object)ContentFinder<Texture2D>.Get("UI/Commands/ForPrisoners");
		...
		if (def.building.bed_canBeMedical)
		{
			...
			command_Toggle2.icon = (Texture)(object)ContentFinder<Texture2D>.Get("UI/Commands/AsMedical");
	...
	public override void DrawGUIOverlay()
	{
		...
		if (OwnersForReading.Count == 1)
		{
			...
			{
				GenMapUI.DrawThingLabel((Thing)this, pawn.LabelShort, defaultThingLabelColor);
			}
			return;
		}
		for (int i = 0; i < OwnersForReading.Count; i++)
		{
			...
			if (!pawn2.InBed() || OwnersForReading[i].CurrentBed() != this || !(pawn2.Position == GetSleepingSlotPos(i)))
			{
				...
				GenMapUI.DrawThingLabel(Vector2.op_Implicit(GetMultiOwnersLabelScreenPosFor(i)), pawn2.LabelShort, defaultThingLabelColor);
			}
		}
	}
	...
	private Vector3 GetMultiOwnersLabelScreenPosFor(int slotIndex)
	{
		...
		Vector2 val = drawPos.MapToUIPosition();
		if (!base.Rotation.IsHorizontal && SleepingSlotsCount == 2)
		{
			val = Vector2.op_Implicit(AdjustOwnerLabelPosToAvoidOverlapping(Vector2.op_Implicit(val), slotIndex));
		}
	...
	private Vector3 AdjustOwnerLabelPosToAvoidOverlapping(Vector3 screenPos, int slotIndex)
	{
		...
		Vector2 val = DrawPos.MapToUIPosition();
		float num2 = Mathf.Abs(screenPos.x - val.x);
		IntVec3 sleepingSlotPos = GetSleepingSlotPos(slotIndex);
		if (num > num2 * 2f)
		{
			float num3 = 0f;
