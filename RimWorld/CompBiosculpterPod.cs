...
public class CompBiosculpterPod : ThingComp, ISuspendableThingHolder, IThingHolder, IThingHolderWithDrawnPawn, IStoreSettingsParent, INotifyHauledTo, ISearchableContents
{
	...
	private Texture2D AutoAgeReversalIcon
	{
		get
		{
			if ((Object)(object)cachedAutoAgeReverseIcon == (Object)null)
			{
	...
	public override void DrawGUIOverlay()
	{
		...
		{
			GenMapUI.DrawThingLabel((Thing)parent, biotunedTo?.LabelShort ?? Occupant.LabelShort, GenMapUI.DefaultThingLabelColor);
		}
	}
	...
	public override IEnumerable<Gizmo> CompGetGizmosExtra()
	{
		...
		foreach (CompBiosculpterPod_Cycle cycle in AvailableCycles)
		{
			...
			command_Action.icon = (Texture)(object)cycle.Props.Icon;
		...
		if (state == BiosculpterPodState.Occupied)
		{
			...
			command_Action2.icon = (Texture)(object)InterruptCycleIcon;
		...
		command_Toggle.icon = (Texture)(object)(autoLoadNutrition ? TexCommand.ForbidOff : TexCommand.ForbidOn);
		...
		if ((biotunedTo?.Ideo?.HasPrecept(PreceptDefOf.AgeReversal_Demanded)).GetValueOrDefault())
		{
			...
			command_Toggle2.icon = (Texture)(object)AutoAgeReversalIcon;
	...
	public override void PostDraw()
	{
		...
		((Vector3)(ref val))._002Ector(parent.def.graphicData.drawSize.x * 0.8f, 1f, parent.def.graphicData.drawSize.y * 0.8f);
		Vector3 drawPos = parent.DrawPos;
		drawPos.y -= 0.07317074f;
		Graphics.DrawMesh(MeshPool.plane10, Matrix4x4.TRS(drawPos, rotation.AsQuat, val), BackgroundMat, 0);
		if (State == BiosculpterPodState.Occupied)
		{
			Pawn occupant = Occupant;
