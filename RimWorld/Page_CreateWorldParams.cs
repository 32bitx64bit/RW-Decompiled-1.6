...
public class Page_CreateWorldParams : Page
{
	...
	public override void DoWindowContents(Rect rect)
	{
		...
		if (ModsConfig.BiotechActive)
		{
			...
			((Rect)(ref rect9))._002Ector(200f, num2, num3, 30f);
		...
		if (!TutorSystem.TutorialMode)
		{
			...
			if (Widgets.ButtonText(new Rect(200f, num2, num3, 30f), "Edit".Translate() + "..."))
			{
				Find.WindowStack.Add(new Dialog_AdvancedGameConfig());
			}
		}
		Widgets.EndGroup();
		WorldFactionsUIUtility.DoWindowContents(new Rect(((Rect)(ref mainRect)).x + ((Rect)(ref mainRect)).xMax - num, ((Rect)(ref mainRect)).y, num, ((Rect)(ref mainRect)).height), isDefaultFactionCounts: factions.SetsEqual(initialFactions), factions: factions);
		float num4 = ((Rect)(ref rect)).yMax - 38f;
		float x = ((Rect)(ref mainRect)).center.x;
		Rect rect11 = default(Rect);
		((Rect)(ref rect11))._002Ector(x - Page.BottomButSize.x - 8.5f, num4, Page.BottomButSize.x, Page.BottomButSize.y);
		if (Widgets.ButtonText(rect11, "ResetAll".Translate()))
		{
			Reset();
		}
		((Rect)(ref rect11)).x = x + 8.5f;
