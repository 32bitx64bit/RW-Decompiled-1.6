...
public class Dialog_ChooseMemes : Window
{
	...
	public override void DoWindowContents(Rect rect)
	{
		...
		Rect outRect = rect;
		((Rect)(ref outRect)).height = ((Rect)(ref outRect)).height - ButSize.y;
		...
		Widgets.Label(new Rect(((Rect)(ref outRect)).x, ((Rect)(ref outRect)).y, ((Rect)(ref rect)).width, 30f), label);
		Text.Font = GameFont.Small;
		((Rect)(ref outRect)).yMin = ((Rect)(ref outRect)).yMin + 30f;
		...
		if (Widgets.ButtonText(new Rect((float)((int)(((Rect)(ref rect)).width - ButSize.x) / 2), ((Rect)(ref rect)).height - ButSize.y, ButSize.x, ButSize.y), "Randomize".Translate()))
		{
		...
		((Rect)(ref rect2))._002Ector(((Rect)(ref rect)).width - ButSize.x, ((Rect)(ref rect)).height - ButSize.y, ButSize.x, ButSize.y);
		...
		((Rect)(ref rect3))._002Ector(((Rect)(ref rect)).xMax - ButSize.x - 240f - 6f, ((Rect)(ref rect2)).y, 240f, ButSize.y);
		if (text != null)
		{
	...
	private void DoMemeSelector(Rect viewRect, MemeCategory category, ref float curY)
	{
		...
		Widgets.Label(((Rect)(ref viewRect)).x, ref curY, ((Rect)(ref viewRect)).width, text);
		curY += 27f;
	...
	private void DoStructureMemeSelector(Rect viewRect, ref float curY, List<MemeDef> memes)
	{
		...
		float num = curY;
		int num2 = Mathf.FloorToInt(((Rect)(ref viewRect)).width / (IdeoUIUtility.MemeBoxSize.x + 8f));
		float num3 = 8f;
		int num4 = memes.Count((MemeDef x) => x.groupDef == null);
		if (num4 < num2)
		{
			num3 = (((Rect)(ref viewRect)).width - IdeoUIUtility.MemeBoxSize.x * (float)(num4 + 1)) / (float)num4;
		}
		memeGroups.Clear();
		Rect memeBox = default(Rect);
		for (int i = 0; i < memes.Count; i++)
		{
			...
			else
			{
				...
				float num8 = (((Rect)(ref viewRect)).width - (float)num7 * IdeoUIUtility.MemeBoxSize.x - (float)(num7 - 1) * num3) / 2f;
				((Rect)(ref memeBox))._002Ector(num8 + (float)num6 * IdeoUIUtility.MemeBoxSize.x + (float)num6 * num3, curY + (float)num5 * IdeoUIUtility.MemeBoxSize.y + (float)num5 * num3, IdeoUIUtility.MemeBoxSize.x, IdeoUIUtility.MemeBoxSize.y);
				DrawMeme(memeDef, memeBox, drawHighlight: false);
				num = Mathf.Max(num, ((Rect)(ref memeBox)).yMax);
			}
		}
		if (memeGroups.Any())
		{
			...
			{
				Rect val = DrawMemeGroup(memeGroup.Value, new Vector2(((Rect)(ref viewRect)).x + ((Rect)(ref viewRect)).width * memeGroup.Key.drawOffset.x, ((Rect)(ref viewRect)).y + num9 + ((Rect)(ref viewRect)).height * memeGroup.Key.drawOffset.y), memeGroup.Key.maxRows);
				num = Mathf.Max(num, ((Rect)(ref val)).yMax);
			}
		}
		memeGroups.Clear();
		GUI.color = Color.white;
	...
	private void DoNormalMemeSelector(Rect viewRect, ref float curY, List<MemeDef> memes)
	{
		...
		int num = Mathf.FloorToInt(((Rect)(ref viewRect)).width / (IdeoUIUtility.MemeBoxSize.x + gapBetweenBoxes));
		int impact;
		...
		for (impact = 1; impact <= 3; impact++)
		{
			...
			((Rect)(ref rect))._002Ector(((Rect)(ref viewRect)).x, curY, ((Rect)(ref viewRect)).width, 30f);
			Widgets.Label(rect, "IdeoImpact".Translate() + ": " + IdeoImpactUtility.MemeImpactLabel(impact).CapitalizeFirst());
			curY = ((Rect)(ref rect)).yMax;
			GUI.color = Color.gray;
			Widgets.DrawLineHorizontal(((Rect)(ref rect)).x, curY - 7f, ((Rect)(ref rect)).width);
			GUI.color = Color.white;
		...
		void DrawRow(ref float y)
		{
			...
			{
				float num4 = (((Rect)(ref viewRect)).width - (float)memesCurrentRow.Count * (IdeoUIUtility.MemeBoxSize.x + gapBetweenBoxes)) / 2f;
				...
				for (int i = 0; i < memesCurrentRow.Count; i++)
				{
					...
					Rect memeBox = GenUI.Rounded(new Rect(((Rect)(ref viewRect)).x + num4, y, IdeoUIUtility.MemeBoxSize.x, IdeoUIUtility.MemeBoxSize.y));
					DrawMeme(memeDef, memeBox, drawHighlight: false);
					num4 = ((Rect)(ref memeBox)).xMax;
				}
				memesCurrentRow.Clear();
	...
	private Rect DrawMemeGroup(List<MemeDef> memes, Vector2 offset, int maxRows)
	{
		...
		((Rect)(ref val))._002Ector(offset.x, offset.y, (float)num * IdeoUIUtility.MemeBoxSize.x, (float)num2 * IdeoUIUtility.MemeBoxSize.y);
