	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		if (targetPawn == null)
		{
			yield return new Command_Action
			{
				...
				icon = (Texture)(object)ImplantTex.Texture,
				action = delegate
				{
					...
					foreach (Pawn item in base.Map.mapPawns.AllPawnsSpawned)
					{
						...
						if (!pawn.IsQuestLodger() && pawn.genes != null && (pawn.IsColonistPlayerControlled || pawn.IsPrisonerOfColony || pawn.IsSlaveOfColony || (pawn.IsColonySubhuman && pawn.IsGhoul)))
						{
							...
							if (num < GeneTuning.BiostatRange.TrueMin)
							{
								list.Add(new FloatMenuOption(string.Concat(pawn.LabelShortCap + ": " + "ResultingMetTooLow".Translate() + " (", num.ToString(), ")"), (Action)null, (Thing)pawn, Color.white, MenuOptionPriority.Default, (Action<Rect>)null, (Thing)null, 0f, (Func<Rect, bool>)null, (WorldObject)null, playSelectionSound: true, 0));
							}
							else if (PawnIdeoDisallowsImplanting(pawn))
							{
								list.Add(new FloatMenuOption((string)(pawn.LabelShortCap + ": " + "IdeoligionForbids".Translate()), (Action)null, (Thing)pawn, Color.white, MenuOptionPriority.Default, (Action<Rect>)null, (Thing)null, 0f, (Func<Rect, bool>)null, (WorldObject)null, playSelectionSound: true, 0));
							}
							else
							{
								list.Add(new FloatMenuOption(pawn.LabelShortCap + ", " + pawn.genes.XenotypeLabelCap, (Action)delegate
								{
									SetTargetPawn(pawn);
		...
		yield return new Command_Action
		{
			...
			icon = (Texture)(object)CancelIcon,
			action = delegate
			{
				Bill bill = targetPawn.BillStack?.Bills.FirstOrDefault((Bill x) => x.xenogerm == this);
				if (bill != null)
