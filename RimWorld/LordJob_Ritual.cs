...
public class LordJob_Ritual : LordJob_Joinable_Gathering
{
	...
	public override StateGraph CreateGraph()
	{
		...
		{
			LordToil_Ritual lordToil_Ritual2 = toils[j];
			RitualStage stage = stages[j];
			int iCapture = j;
			...
			if (!stage.failTriggers.NullOrEmpty())
			{
				foreach (StageFailTrigger f in stage.failTriggers)
				{
					...
					transition3.AddTrigger(new Trigger_TickCondition(() => ticksPassed > f.allowanceTicks && f.Failed(this, selectedTarget, SecondFocusForStage(i1))));
			...
			{
				list.Add(endTrigger.MakeTrigger(this, selectedTarget, AllSecondFoci(j), stage));
			}
			bool flag = j == stages.Count - 1;
			Transition transition4 = new Transition(lordToil_Ritual2, flag ? ((LordToil)lordToil_End) : ((LordToil)toils[j + 1]));
		...
		float Progress()
		{
			float num = (float)ticksPassed / (float)durationTicks;
			if (lastEssentialStageIndex != -1)
			{
				int num2 = base.DurationTicks - lastEssentialStageEndedTick;
				num = (float)ticksSinceLastEssentialStage / (float)num2;
			}
	...
	public override IEnumerable<Gizmo> GetPawnGizmos(Pawn p)
	{
		if (p != organizer && !assignments.ExtraRequiredPawnsForReading.Contains(p) && (!TryGetRoleFor(p, out var role) || !role.required))
		{
			Command_Action command_Action = new Command_Action
			{
				...
				icon = (Texture)(object)ritual.CancelIcon
			};
	...
	public Gizmo GetCancelGizmo()
	{
		...
		command_Action.icon = (Texture)(object)ritual.CancelIcon;
		if (!ritual.def.mergeRitualGizmosFromAllIdeos)
		{
			command_Action.defaultIconColor = ritual.ideo.Color;
		}
	...
	public IntVec3 CurrentSpectatorCrowdCenter()
	{
		...
		val /= (float)num;
		return new IntVec3((int)val.x, (int)val.y, (int)val.z);
	}
