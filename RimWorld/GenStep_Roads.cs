...
public class GenStep_Roads : GenStep
{
	...
	private List<NeededRoad> CalculateNeededRoads(Map map)
	{
		...
		if (list2.Count > 1)
		{
			...
			{
				val += Vector3Utility.HorizontalVectorFromAngle(item2.angle);
			}
			val /= (float)(-list2.Count);
			val += Rand.UnitVector3 * 1f / 6f;
			val.y = 0f;
			for (int i = 0; i < list2.Count; i++)
			{
				list2[i] = new NeededRoad
				{
					angle = (Vector3Utility.HorizontalVectorFromAngle(list2[i].angle) + val).AngleFlat(),
					road = list2[i].road
				};
			}
		}
	...
	private GenMath.BezierCubicControls GenerateBezierControls(List<IntVec3> path, int pathStartIndex, int pathEndIndex)
	{
		...
		Vector3 val = path[pathStartIndex].ToVector3Shifted();
		Vector3 val2 = (path[pathEndIndex] - path[index]).ToVector3();
		result.w1 = val + ((Vector3)(ref val2)).normalized * 4f;
		Vector3 val3 = path[pathEndIndex].ToVector3Shifted();
		val2 = (path[pathStartIndex] - path[index2]).ToVector3();
		result.w2 = val3 + ((Vector3)(ref val2)).normalized * 4f;
	...
	private void ApplyDistanceField(DistanceElement[,] distance, Map map, TerrainDef rockDef, RoadDef roadDef, RoadPathingDef pathingDef)
	{
		for (int i = 0; i < map.Size.x; i++)
		{
			for (int j = 0; j < map.Size.z; j++)
			{
				...
				float num = Mathf.Abs(distanceElement.fromRoad + Rand.Value - 0.5f);
				for (int k = 0; k < roadDef.roadGenSteps.Count; k++)
				{
					RoadDefGenStep roadDefGenStep = roadDef.roadGenSteps[k];
					float x = Mathf.LerpUnclamped(distanceElement.fromRoad, num, roadDefGenStep.antialiasingMultiplier);
					float num2 = roadDefGenStep.chancePerPositionCurve.Evaluate(x);
					if (!(num2 <= 0f) && (roadDefGenStep.periodicSpacing == 0 || !(distanceElement.alongPath / (float)roadDefGenStep.periodicSpacing % 1f * (float)roadDefGenStep.periodicSpacing >= 1f)))
					{
	...
	private void FillDistanceField(DistanceElement[,] field, float cx, float cz, float alongPath, float radius, Map map)
	{
		...
		IntVec3 origin = IntVec3Utility.ToIntVec3(new Vector3(cx, 0f, cz)).ClampInsideMap(map);
