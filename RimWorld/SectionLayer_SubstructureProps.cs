...
public class SectionLayer_SubstructureProps : SectionLayer
{
	...
	private void AddQuad(LayerSubMesh sm, IntVec3 c, float altitude, Rot4 rotation, bool addGravshipMask)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			sm.uvs.Add(Vector2.op_Implicit(UVs[(num + i) % 4]));
		}
		sm.tris.Add(count);
		sm.tris.Add(count + 1);
		sm.tris.Add(count + 2);
		...
		if (addGravshipMask)
		{
			Material material = MaterialPool.MatFrom((Texture2D)/*isinst with value type is only supported in some contexts*/, color: sm.material.color, shader: ShaderDatabase.GravshipMaskMasked);
