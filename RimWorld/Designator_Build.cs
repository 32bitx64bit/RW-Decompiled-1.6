...
public class Designator_Build : Designator_Place
{
	...
	private void UpdateIcon()
	{
		...
		icon = (Texture)(object)entDef.GetUIIconForStuff(StuffDef);
	...
	public Texture ResolvedIcon(ThingStyleDef styleDef = null)
	{
		...
		if (entDef is ThingDef thingDef)
		{
			...
			return (Texture)(object)Widgets.GetIconFor(thingDef, StuffDef, thingStyleDef);
		}
		if (StuffDef != null && entDef.graphic is Graphic_Appearances graphic_Appearances)
		{
			return (Texture)(Texture2D)graphic_Appearances.SubGraphicFor(StuffDef).MatAt(entDef.defaultPlacingRot).mainTexture;
		}
		return icon;
	}
	...
	protected override void DrawPlaceMouseAttachments(float curX, ref float curY)
	{
		...
		if (((Rect)(ref infoRect)).Contains(UI.MousePositionOnUIInverted))
		{
			return;
		}
		if (eyedropper != null)
		{
			...
			((Rect)(ref rect))._002Ector(curX, curY, 9999f, Text.LineHeight * 2f);
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			float num2 = curY;
			Widgets.ThingIcon(new Rect(curX, num2, 27f, 27f), thingDefCountClass.thingDef);
			Rect rect2 = new Rect(curX + 29f, num2, 999f, 29f);
	...
	public override void DrawPanelReadout(ref float curY, float width)
	{
		...
		for (int i = 0; i < tmpcostList.Count; i++)
		{
			...
			float num = width - 60f;
			float num2 = Text.CalcHeight(text, num) - 5f;
			Widgets.Label(new Rect(60f, curY + 2f, num, num2 + 5f), text);
			curY += num2;
		}
		...
		if (styleCategoryDef != null)
		{
			...
			Rect rect = default(Rect);
			((Rect)(ref rect))._002Ector(0f, curY + 2f, width, Text.CalcHeight(taggedString, width));
		...
		if (!flag)
		{
			...
			((Rect)(ref rect2))._002Ector(0f, curY + 2f, width, Text.CalcHeight(taggedString2, width));
	...
	private void DrawSkillRequirement(SkillDef skillDef, int requirement, float width, ref float curY)
	{
		...
		Rect rect = default(Rect);
		((Rect)(ref rect))._002Ector(0f, curY + 2f, width, 24f);
