...
public class HumanEmbryo : GeneSetHolderBase
{
	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		if (implantTarget == null)
		{
			...
			Command_Action command_Action = new Command_Action
			{
				...
				icon = (Texture)(object)ImplantIcon.Texture,
				action = delegate
				{
			...
			Command_Action command_Action2 = new Command_Action
			{
				...
				icon = (Texture)(object)Building_GrowthVat.InsertEmbryoIcon.Texture,
		...
		else if (base.Spawned)
		{
			...
			command_Action3.icon = (Texture)(object)CancelImplantIcon;
	...
	private FloatMenuOption CanImplantFloatOption(Pawn pawn, bool pawnAsLabel)
	{
		...
		{
			return new FloatMenuOption(pawnAsLabel ? pawn.LabelShort : "ImplantLabel".Translate().ToString(), (Action)delegate
			{
		...
		{
			return new FloatMenuOption(pawnAsLabel ? "DisabledOption".Translate(pawn.LabelShort, acceptanceReport.Reason).ToString() : ("ImplantCannotFloatOption".Translate().ToString() + ": " + acceptanceReport.Reason), (Action)null, (Thing)pawn, Color.white, MenuOptionPriority.Default, (Action<Rect>)null, (Thing)null, 0f, (Func<Rect, bool>)null, (WorldObject)null, playSelectionSound: true, 0);
		}
		return null;
	}
