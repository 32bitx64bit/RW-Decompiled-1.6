...
public static class PawnDiedOrDownedThoughtsUtility
{
	...
	private static void AppendThoughts_Relations(Pawn victim, DamageInfo? dinfo, PawnDiedOrDownedThoughtsKind thoughtsKind, List<IndividualThoughtToAdd> outIndividualThoughts, List<ThoughtToAddToAll> outAllColonistsThoughts)
	{
		...
		{
			Pawn pawn2 = allMapsCaravansAndTravellingTransporters_Alive[i];
			if (pawn2.needs != null && pawn2.RaceProps.IsFlesh && pawn2.needs.mood != null && PawnUtility.ShouldGetThoughtAbout(pawn2, victim))
			{
				int num2 = pawn2.relations.OpinionOf(victim);
				if (num2 >= 20)
				{
					outIndividualThoughts.Add(new IndividualThoughtToAdd(thoughtDef3, pawn2, victim, victim.relations.GetFriendDiedThoughtPowerFactor(num2)));
				}
				else if (num2 <= -20)
				{
					outIndividualThoughts.Add(new IndividualThoughtToAdd(thoughtDef4, pawn2, victim, victim.relations.GetRivalDiedThoughtPowerFactor(num2)));
				}
			}
		}
		void GiveThoughtsForAnimalBond(ThoughtDef thoughtDef)
		{
			...
			{
				if (directRelations[j].otherPawn.needs != null && directRelations[j].otherPawn.needs.mood != null && PawnUtility.ShouldGetThoughtAbout(directRelations[j].otherPawn, victim) && directRelations[j].def == PawnRelationDefOf.Bond)
				{
					outIndividualThoughts.Add(new IndividualThoughtToAdd(thoughtDef, directRelations[j].otherPawn, victim));
				}
			}
		}
	}
	...
	private static Pawn FindResponsibleColonist(Pawn victim, DamageInfo? dinfo)
	{
		...
		if (victim.Spawned)
		{
			...
			{
				return GenCollection.MinBy(victim.Map.mapPawns.FreeColonistsSpawned.Where((Pawn x) => !x.Downed), (Pawn x) => x.Position.DistanceToSquared(victim.Position));
			}
			if (victim.Map.mapPawns.FreeColonistsSpawned.Any())
			{
				return GenCollection.MinBy(victim.Map.mapPawns.FreeColonistsSpawned, (Pawn x) => x.Position.DistanceToSquared(victim.Position));
			}
		}
