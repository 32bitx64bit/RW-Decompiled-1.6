...
public class WorldRoutePlanner
{
	...
	private void DoRouteDetailsBox()
	{
		...
		if (Current.ProgramState == ProgramState.Entry)
		{
			...
			((Rect)(ref reference)).y = ((Rect)(ref reference)).y - 22f;
		}
		Find.WindowStack.ImmediateWindow(1373514241, rect, WindowLayer.Dialog, delegate
		{
			...
			if (active)
			{
				...
				{
					Widgets.Label(new Rect(0f, num, ((Rect)(ref rect)).width, 25f), "RoutePlannerEstTimeToFinalDest".Translate(GetTicksToWaypoint(waypoints.Count - 1).ToStringTicksToDays("0.#")));
				}
				else if (cantRemoveFirstWaypoint)
				{
					Widgets.Label(new Rect(0f, num, ((Rect)(ref rect)).width, 25f), "RoutePlannerAddOneOrMoreWaypoints".Translate());
				}
				else
				{
					Widgets.Label(new Rect(0f, num, ((Rect)(ref rect)).width, 25f), "RoutePlannerAddTwoOrMoreWaypoints".Translate());
				}
				num += 20f;
				if (!CaravanInfo.HasValue || !CaravanInfo.Value.pawns.Any())
				{
					GUI.color = new Color(0.8f, 0.6f, 0.6f);
					Widgets.Label(new Rect(0f, num, ((Rect)(ref rect)).width, 25f), "RoutePlannerUsingAverageTicksPerMoveWarning".Translate());
				}
				else if (currentFormCaravanDialog == null && CaravanAtTheFirstWaypoint != null)
				{
					GUI.color = Color.gray;
					Widgets.Label(new Rect(0f, num, ((Rect)(ref rect)).width, 25f), "RoutePlannerUsingTicksPerMoveOfCaravan".Translate(CaravanAtTheFirstWaypoint.LabelCap));
				}
				num += 20f;
				GUI.color = Color.gray;
				Widgets.Label(new Rect(0f, num, ((Rect)(ref rect)).width, 25f), SteamDeck.IsSteamDeckInNonKeyboardMode ? "RoutePlannerPressRMBToAddAndRemoveWaypointsController".Translate() : "RoutePlannerPressRMBToAddAndRemoveWaypoints".Translate());
				num += 20f;
				if (currentFormCaravanDialog != null)
				{
					Widgets.Label(new Rect(0f, num, ((Rect)(ref rect)).width, 25f), SteamDeck.IsSteamDeckInNonKeyboardMode ? "RoutePlannerPressEscapeToReturnToCaravanFormationDialogController".Translate().Resolve().AdjustedForKeys() : "RoutePlannerPressEscapeToReturnToCaravanFormationDialog".Translate().Resolve());
				}
				else
				{
					Widgets.Label(new Rect(0f, num, ((Rect)(ref rect)).width, 25f), SteamDeck.IsSteamDeckInNonKeyboardMode ? "RoutePlannerPressEscapeToExitController".Translate().Resolve().AdjustedForKeys() : "RoutePlannerPressEscapeToExit".Translate().Resolve());
				}
				num += 20f;
				GUI.color = Color.white;
	...
	private void DoTileTooltips()
	{
		...
		for (int i = 0; i < paths.Count; i++)
		{
			if (paths[i].NodesReversed.Contains(planetTile))
			{
				...
				Text.Font = GameFont.Small;
				Vector2 val = Text.CalcSize(str);
				val.x += 20f;
				val.y += 20f;
				Vector2 mouseAttachedWindowPos = GenUI.GetMouseAttachedWindowPos(val.x, val.y);
				Rect rect = new Rect(mouseAttachedWindowPos, val);
	...
	public void DoRoutePlannerButton(ref float curBaseY)
	{
		...
		float num = 57f;
		float num2 = 33f;
		Rect val = new Rect((float)UI.screenWidth - 10f - num, curBaseY - 10f - num2, num, num2);
		if (Widgets.ButtonImage(val, ButtonTex, Color.white, new Color(0.8f, 0.8f, 0.8f)))
		{
			if (active)
			{
				Stop();
		...
		TooltipHandler.TipRegionByKey(val, "RoutePlannerButtonTip");
		curBaseY -= num2 + 20f;
	}
