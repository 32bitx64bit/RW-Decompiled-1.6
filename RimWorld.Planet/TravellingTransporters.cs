...
public class TravellingTransporters : WorldObject, IThingHolder
{
	...
	public override float ExpandingIconRotation
	{
		get
		{
			...
			Vector2 val = GenWorldUI.WorldToUIPosition(Start);
			Vector2 val2 = GenWorldUI.WorldToUIPosition(End);
			float num = Mathf.Atan2(val2.y - val.y, val2.x - val.x) * 57.29578f;
			if (num > 180f)
			{
				num -= 180f;
			}
	...
	private float TraveledPctStepPerTick
	{
		get
		{
			...
			float num = GenMath.SphericalDistance(((Vector3)(ref start)).normalized, ((Vector3)(ref end)).normalized);
			if (num == 0f)
			{
				return 1f;
			}
