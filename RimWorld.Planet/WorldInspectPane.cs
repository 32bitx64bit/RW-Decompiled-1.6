...
public class WorldInspectPane : Window, IInspectPane
{
	...
	private string TileInspectString
	{
		get
		{
			...
			Vector2 val = Find.WorldGrid.LongLatOf(SelectedTile);
			stringBuilder.Append(val.y.ToStringLatitude());
			stringBuilder.Append(" ");
			stringBuilder.Append(val.x.ToStringLongitude());
			...
			if (tile is SurfaceTile surfaceTile)
			{
				...
				{
					stringBuilder.AppendLine();
					stringBuilder.Append(GenCollection.MaxBy(surfaceTile.Roads.Select((SurfaceTile.RoadLink rl) => rl.road), (RoadDef road) => road.priority).LabelCap);
				}
	...
	protected override void SetInitialSizeAndPosition()
	{
		...
		((Rect)(ref windowRect)).y = PaneTopY;
	}
	...
	public void DoInspectPaneButtons(Rect rect, ref float lineEndWidth)
	{
		...
		{
			float x = ((Rect)(ref rect)).width - 48f;
