...
public static class WorldGizmoUtility
{
	...
	public static Gizmo GetJumpToGizmo()
	{
		return new Command_Action
		{
			...
			icon = (Texture)(object)JumpToCommand,
			action = delegate
			{
				...
				foreach (Map i in Find.Maps)
				{
					if (!tmpJumpTargets.Contains(i.Parent) && !i.IsPocketMap)
					{
						string text = i.Parent.LabelCap;
						if (GravshipUtility.TryGetNameOfGravshipOnMap(i, out var name))
						{
							text = text + " (" + name + ")";
						}
						list.Add(new FloatMenuOption(text, delegate
						{
							CameraJumper.TryJumpAndSelect(i.Parent);
						}, i.Parent.ExpandingIcon, i.Parent.ExpandingIconColor));
						tmpJumpTargets.Add(i.Parent);
					}
				}
	...
	private static Command_Action GetFormCaravanAction(string label, string desc, Action action)
	{
		return new Command_Action
		{
			...
			defaultLabel = label,
			defaultDesc = desc,
			icon = (Texture)(object)FormCaravanCommand,
