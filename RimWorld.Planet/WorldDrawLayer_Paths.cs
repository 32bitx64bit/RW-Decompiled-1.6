...
public abstract class WorldDrawLayer_Paths : WorldDrawLayer
{
	...
	private void AddPathEndpoint(LayerSubMesh subMesh, List<Vector3> verts, int index, Color32 color, PlanetTile tileID, OutputDirection data)
	{
		...
		Vector3 val = FinalizePoint(verts[index], data.distortionFrequency, data.distortionIntensity * num);
		Vector3 val2 = FinalizePoint(verts[index2], data.distortionFrequency, data.distortionIntensity * num);
		subMesh.verts.Add(Vector3.LerpUnclamped(val, val2, 0.5f - data.width));
		subMesh.colors.Add(color.MutateAlpha(0));
		subMesh.verts.Add(Vector3.LerpUnclamped(val, val2, 0.5f));
		subMesh.colors.Add(color);
		subMesh.verts.Add(Vector3.LerpUnclamped(val, val2, 0.5f + data.width));
		subMesh.colors.Add(color.MutateAlpha(0));
	}
