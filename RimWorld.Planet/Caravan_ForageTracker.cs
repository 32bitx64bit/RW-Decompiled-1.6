...
public class Caravan_ForageTracker : IExposable
{
	...
	private void Forage()
	{
		...
		if (foragedFood != null)
		{
			int num = GenMath.RoundRandom(ForagedFoodPerDayCalculator.GetForagedFoodCountPerInterval(caravan));
			int num2 = Mathf.FloorToInt((caravan.MassCapacity - caravan.MassUsage) / foragedFood.GetStatValueAbstract(StatDefOf.Mass));
			num = Mathf.Min(num, num2);
			while (num > 0)
			{
				Thing thing = ThingMaker.MakeThing(foragedFood);
				thing.stackCount = Mathf.Min(num, foragedFood.stackLimit);
