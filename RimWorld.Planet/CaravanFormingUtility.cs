...
public static class CaravanFormingUtility
{
	...
	public static void LeadAnimalsToPen(List<Pawn> pawns)
	{
		...
		if (tmpNeedRoping.Any() && tmpRopers.Any())
		{
			foreach (Pawn ropee in tmpNeedRoping)
			{
				...
				{
					GenCollection.MinBy(tmpRopers, (Pawn p) => p.Position.DistanceToSquared(ropee.Position)).roping.RopePawn(ropee);
				}
			}
	...
	public static IEnumerable<Gizmo> GetGizmos(Pawn pawn)
	{
		if (IsFormingCaravanOrDownedPawnToBeTakenByCaravan(pawn))
		{
			...
			command_Action.icon = (Texture)(object)TexCommand.ClearPrioritizedWork;
			...
			command_Action2.icon = (Texture)(object)RemoveFromCaravanCommand;
			...
			command_Action3.icon = (Texture)(object)ForceDepartCaravanCommand;
		...
		else
		{
			...
			command_Action4.icon = (Texture)(object)AddToCaravanCommand;
