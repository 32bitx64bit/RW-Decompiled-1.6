...
public static class DaysWorthOfFoodCalculator
{
	...
	private static float ApproxDaysWorthOfFood(List<Pawn> pawns, List<ThingDefCount> extraFood, PlanetTile tile, IgnorePawnsInventoryMode ignoreInventory, Faction faction, WorldPath path = null, float nextTileCostLeft = 0f, int caravanTicksPerMove = 3300, bool assumeCaravanMoving = true)
	{
		...
		{
			Pawn pawn3 = pawns[j];
			if (InventoryCalculatorsUtility.ShouldIgnoreInventoryOf(pawn3, ignoreInventory))
			{
				continue;
			}
			ThingOwner<Thing> innerContainer = pawn3.inventory.innerContainer;
			int k = 0;
		...
		{
			Pawn pawn4 = pawns[num];
			if (pawn4.RaceProps.EatsFood && pawn4.needs.food != null)
			{
				Need_Food food = pawn4.needs.food;
		...
		foreach (Pawn pawn2 in pawns)
		{
			if (ChildcareUtility.CanSuckle(pawn2, out var _))
			{
				...
				int num7 = tmpLactatingPawns.FindIndex((Pawn feeder) => ChildcareUtility.CanMomBreastfeedBaby(feeder, pawn2, out reason2) && pawn2.mindState.AutofeedSetting(feeder) != AutofeedMode.Never);
				if (num7 >= 0)
				{
					tmpLactatingPawns[num7] = null;
					babiesWithFeeders.Add(pawn2);
				}
			}
		}
		bool flag3;
		do
		{
			...
			for (int count9 = pawns.Count; num13 < count9; num13++)
			{
				...
				else
				{
					...
					do
					{
						int num16 = BestEverEdibleFoodIndexFor(pawn5, tmpFood);
						if (num16 < 0)
						{
							if (tmpDaysWorthOfFoodForPawn[num13] < num3)
							{
