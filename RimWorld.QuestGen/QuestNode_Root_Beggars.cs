...
public class QuestNode_Root_Beggars : QuestNode
{
	...
	protected override void RunInt()
	{
		...
		for (int i = 0; i < num; i++)
		{
			DevelopmentalStage developmentalStage = ((i >= num - num2) ? DevelopmentalStage.Child : DevelopmentalStage.Adult);
	...
	public static int LodgerCountFromPopulation(Slate slate, Map map)
	{
		...
		{
			float num3 = Mathf.InverseLerp((float)PopulationCountToCurve.TrueMin, (float)PopulationCountToCurve.TrueMax, (float)num);
			FloatRange floatRange = new FloatRange(Mathf.Lerp(LodgerCountBasedOnColonyPopulationFactorRange_Low.TrueMin, LodgerCountBaseOnColonyPopulationFactorRange_High.TrueMin, num3), Mathf.Lerp(LodgerCountBasedOnColonyPopulationFactorRange_Low.TrueMax, LodgerCountBaseOnColonyPopulationFactorRange_High.TrueMax, num3));
