...
public class SampleOneShot : Sample
{
	...
	public static SampleOneShot TryMakeAndPlay(SubSoundDef def, AudioClip clip, SoundInfo info)
	{
		...
		if ((Object)(object)sampleOneShot.source == (Object)null)
		{
			return null;
		}
		...
		if (!def.onCamera)
		{
			((Component)sampleOneShot.source).gameObject.transform.position = info.Maker.Cell.ToVector3ShiftedWithAltitude(0f);
			...
			if (def.canVacuumDampen)
			{
				...
				if (map != null && map.Biome?.inVacuum == true)
				{
					...
					AudioSource obj = sampleOneShot.source;
					obj.volume *= Mathf.Lerp(1f, 0.05f, num);
