...
internal class TarDirectory : VirtualDirectory
{
	...
	private void CheckLazyLoad()
	{
		...
		using (FileStream fileStream = File.OpenRead(lazyLoadArchive))
		{
			TarInputStream val = new TarInputStream((Stream)fileStream);
			try
			{
				ParseTAR(this, val, lazyLoadArchive);
			}
			finally
			{
	...
	private static void ReadTarEntryData(TarInputStream tarIn, Stream outStream, byte[] buffer = null)
	{
		...
		for (int num = ((Stream)(object)tarIn).Read(buffer, 0, buffer.Length); num > 0; num = ((Stream)(object)tarIn).Read(buffer, 0, buffer.Length))
		{
