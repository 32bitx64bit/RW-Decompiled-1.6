...
public class FloatMenu : Window
{
	...
	public override void ExtraOnGUI()
	{
		...
		if (!title.NullOrEmpty())
		{
			...
			((Vector2)(ref val))._002Ector(((Rect)(ref windowRect)).x, ((Rect)(ref windowRect)).y);
			Text.Font = GameFont.Small;
			float num = Mathf.Max(150f, 15f + Text.CalcSize(title).x);
			Rect titleRect = new Rect(val.x + TitleOffset.x, val.y + TitleOffset.y, num, 23f);
			Find.WindowStack.ImmediateWindow(6830963, titleRect, WindowLayer.Super, delegate
			{
				...
				GUI.color = baseColor;
				Text.Font = GameFont.Small;
				Rect val2 = titleRect.AtZero();
				((Rect)(ref val2)).width = 150f;
				GUI.DrawTexture(val2, (Texture)(object)TexUI.TextBGBlack);
				Rect rect = titleRect.AtZero();
				((Rect)(ref rect)).x = ((Rect)(ref rect)).x + 15f;
	...
	public override void DoWindowContents(Rect rect)
	{
		...
		for (int i = 0; i < options.Count; i++)
		{
			...
			((Rect)(ref rect2))._002Ector(zero.x, zero.y, columnWidth, requiredHeight);
	...
	private void UpdateBaseColor()
	{
		...
		Rect r = GenUI.ContractedBy(new Rect(0f, 0f, TotalWidth, TotalWindowHeight), -5f);
