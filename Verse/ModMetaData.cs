...
public class ModMetaData : WorkshopUploadable
{
	private class ModMetaDataInternal
	{
		...
		public List<Version> SupportedVersions { get; private set; }
	...
	public Texture2D PreviewImage
	{
		get
		{
			...
			if (File.Exists(PreviewImagePath))
			{
				...
				ImageConversion.LoadImage(previewImage, File.ReadAllBytes(PreviewImagePath));
			}
	...
	public Texture2D Icon
	{
		get
		{
			...
			else if (File.Exists(ModIconImagePath))
			{
				...
				ImageConversion.LoadImage(iconImage, File.ReadAllBytes(ModIconImagePath));
			}
			else
			{
	...
	public bool VersionCompatible
	{
		get
		{
			...
			return meta.SupportedVersions.Any((Version v) => VersionControl.IsCompatible(v));
		}
	}
	...
	public bool MadeForNewerVersion
	{
		get
		{
			...
			return meta.SupportedVersions.Any((Version v) => v.Major > VersionControl.CurrentMajor || (v.Major == VersionControl.CurrentMajor && v.Minor > VersionControl.CurrentMinor));
		}
	}
	...
	public List<Version> SupportedVersionsReadOnly => meta.SupportedVersions;

	IEnumerable<Version> WorkshopUploadable.SupportedVersions => SupportedVersionsReadOnly;
	...
	public void SetPublishedFileId(PublishedFileId_t newPfid)
	{
		...
		if (!(publishedFileIdInt == newPfid))
		{
			...
			File.WriteAllText(PublishedFileIdPath, ((object)(PublishedFileId_t)(ref newPfid)).ToString());
		}
	}
