			...
			list.Add(new DebugActionNode(traverseMode2.ToString(), DebugActionType.Action, delegate
			{
				...
				tool = new DebugTool("from...", delegate
				{
					...
					}, delegate
					{
						...
						if (fromPawn != null)
						{
							flag = fromPawn.CanReach(intVec, PathEndMode.OnCell, Danger.Deadly, canBashDoors: false, canBashFences: false, traverseMode);
							intVec2 = fromPawn.Position;
						}
						else
						{
							flag = Find.CurrentMap.reachability.CanReach(from, intVec, PathEndMode.OnCell, traverseMode, Danger.Deadly);
							intVec2 = from;
						}
