...
public static class DataExposeUtility
{
	...
	public static void LookBoolArray(ref NativeArray<bool> arr, int elements, string label)
	{
		...
		if (Scribe.mode == LoadSaveMode.LoadingVars)
		{
			if (!arr.IsCreated)
			{
				arr = new NativeArray<bool>(elements, (Allocator)4, (NativeArrayOptions)1);
			}
			arr.CopyFrom(arr2);
		}
	}
	...
	public static void LookBitArray(ref NativeBitArray arr, int elements, string label)
	{
		...
		if (Scribe.mode == LoadSaveMode.Saving)
		{
			...
			for (int i = 0; i < ((NativeBitArray)(ref arr)).Length; i++)
			{
				arr2[i] = ((NativeBitArray)(ref arr)).IsSet(i);
			}
		}
		LookBoolArray(ref arr2, elements, label);
		if (Scribe.mode == LoadSaveMode.LoadingVars)
		{
			if (!((NativeBitArray)(ref arr)).IsCreated)
			{
				arr = new NativeBitArray(elements, AllocatorHandle.op_Implicit((Allocator)4), (NativeArrayOptions)1);
			}
			for (int j = 0; j < arr2.Length; j++)
			{
				((NativeBitArray)(ref arr)).Set(j, arr2[j]);
			}
		}
	}
