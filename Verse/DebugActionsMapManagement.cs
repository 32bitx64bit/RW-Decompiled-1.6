	...
	private static List<DebugActionNode> RunMapGenerator()
	{
		...
		foreach (MapGeneratorDef item in DefDatabase<MapGeneratorDef>.AllDefsListForReading)
		{
			...
			list.Add(new DebugActionNode(defLocal.defName)
			{
				action = delegate
				{
					PlanetTile tile2 = Find.WorldGrid.Surface.Tiles.Where((Tile tile) => tile.PrimaryBiome.canBuildBase).RandomElement().tile;
					MapParent mapParent = (MapParent)WorldObjectMaker.MakeWorldObject(tile2.LayerDef.SettlementWorldObjectDef);
					mapParent.Tile = tile2;
					mapParent.SetFaction(Faction.OfPlayer);
					Find.WorldObjects.Add(mapParent);
