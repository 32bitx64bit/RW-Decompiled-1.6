...
public class CompGlower : ThingComp
{
	...
	public override IEnumerable<Gizmo> CompGetGizmosExtra()
	{
		...
		Command_ColorIcon command_ColorIcon = new Command_ColorIcon
		{
			...
			icon = (Texture)(object)ContentFinder<Texture2D>.Get("UI/Commands/ChangeColor"),
			color = color,
			action = delegate
			{
		...
		if (doCopyPasteGizmos)
		{
			...
			command_ColorIcon2.icon = (Texture)(object)ContentFinder<Texture2D>.Get("UI/Commands/CopyColor");
			...
			if (ColorClipboard.HasValue)
			{
				...
				Color.RGBToHSV(Color32.op_Implicit(ColorClipboard.Value.ProjectToColor32()), ref hue, ref sat, ref num);
				flag2 = false;
			}
			Command_ColorIcon command_ColorIcon3 = new Command_ColorIcon();
			command_ColorIcon3.icon = (Texture)(object)ContentFinder<Texture2D>.Get("UI/Commands/PasteColor");
			...
			if (!flag2)
			{
				command_ColorIcon3.color = Color32.op_Implicit(Color.HSVToRGB(hue, sat, 1f));
			}
		...
		{
			icon = (Texture)(object)ContentFinder<Texture2D>.Get(darklight ? "UI/Commands/SetNormalLight" : "UI/Commands/SetDarklight"),
