...
public class MapTemperature : ICellBoolGiver
{
	...
	private Color GetColorForTemperature(float temperature)
	{
		...
		for (int i = 1; i < temperatureColorMap.Count; i++)
		{
			if (temperatureColorMap[i].Item1 > temperature)
			{
				...
				float num = (temperature - item) / (item2 - item);
				return Color.Lerp(temperatureColorMap[index].Item2, temperatureColorMap[index2].Item2, num);
			}
		}
	...
	public void DebugLogTemperatureOverlayColors()
	{
		...
		for (int i = -50; i <= num; i++)
		{
			...
			Color colorForTemperature = GetColorForTemperature(i);
			stringBuilder.AppendLine(text + ": " + ((object)(Color)(ref colorForTemperature)).ToString());
		}
		Log.Message(stringBuilder.ToString());
	}
}
