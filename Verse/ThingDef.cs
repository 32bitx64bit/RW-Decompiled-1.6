	...
	public override IEnumerable<StatDrawEntry> SpecialDisplayStats(StatRequest req)
	{
		...
		if (recipes.Any())
		{
			...
			if (recipeDef != null && !recipeDef.ingredients.NullOrEmpty())
			{
				...
				for (int l = 0; l < recipeDef.ingredients.Count; l++)
				{
					IngredientCount ingredientCount = recipeDef.ingredients[l];
		...
		for (int k = 0; k < comps.Count; k++)
		{
			foreach (StatDrawEntry item7 in comps[k].SpecialDisplayStats(req))
			{
				yield return item7;
			}
		}
		...
		if (!equippedStatOffsets.NullOrEmpty())
		{
			for (int k = 0; k < equippedStatOffsets.Count; k++)
			{
				...
				float num8 = equippedStatOffsets[k].value;
				...
				yield return new StatDrawEntry(StatCategoryDefOf.EquippedStatOffsets, equippedStatOffsets[k].stat, num8, StatRequest.ForEmpty(), ToStringNumberSense.Offset, null, forceUnfinalizedMode: true).SetReportText(stringBuilder7.ToString());
			}
		}
		if (!IsDrug)
		{
