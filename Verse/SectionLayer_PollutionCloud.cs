...
public class SectionLayer_PollutionCloud : SectionLayer_Gas
{
	...
	public override Material Mat
	{
		get
		{
			...
			if ((Object)(object)matCached == (Object)null)
			{
				matCached = MaterialPool.MatFrom(PollutionTex.Texture, ShaderDatabase.PollutionCloud, Color.white, 3000);
				matCached.SetTexture(ShaderPropertyIDs.FadeTex, (Texture)(object)PollutionFadeTex.Texture);
				matCached.SetVector(ShaderPropertyIDs.TexScrollSpeed, Vector4.op_Implicit(FadeTexScrollSpeed));
				matCached.SetVector(ShaderPropertyIDs.TexScale, Vector4.op_Implicit(FadeTexScale));
			}
			return matCached;
		}
	}
