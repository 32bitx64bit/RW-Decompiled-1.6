...
public sealed class FogGrid : IExposable, IDisposable
{
	...
	public FogGrid(Map map)
	{
		...
		this.map = map;
		fogGrid = new NativeBitArray(map.cellIndices.NumGridCells, AllocatorHandle.op_Implicit((Allocator)4), (NativeArrayOptions)1);
	}
	...
	private void UnfogWorker(IntVec3 c)
	{
		...
		if (((NativeBitArray)(ref fogGrid)).IsSet(num))
		{
			((NativeBitArray)(ref fogGrid)).Set(num, false);
	...
	{
		if (!((NativeBitArray)(ref fogGrid)).IsCreated || !c.InBounds(map))
		{
			return false;
		}
		return ((NativeBitArray)(ref fogGrid)).IsSet(map.cellIndices.CellToIndex(c));
	}

	public bool IsFogged(int index)
	{
		if (!((NativeBitArray)(ref fogGrid)).IsCreated)
		{
			return false;
		}
		return ((NativeBitArray)(ref fogGrid)).IsSet(index);
	}
	...
	public void Refog(CellRect rect)
	{
		foreach (IntVec3 item in rect)
		{
			...
			if (!((NativeBitArray)(ref fogGrid)).IsSet(num))
			{
				((NativeBitArray)(ref fogGrid)).Set(num, true);
				if (Current.ProgramState == ProgramState.Playing)
				{
	...
	internal void SetAllFogged()
	{
		...
		{
			((NativeBitArray)(ref fogGrid)).Set(cellIndices.CellToIndex(allCell), true);
		}
		if (Current.ProgramState == ProgramState.Playing)
		{
			map.roofGrid.Drawer.SetDirty();
	...
	public void Dispose()
	{
		((NativeBitArray)(ref fogGrid)).Dispose();
	}
}
