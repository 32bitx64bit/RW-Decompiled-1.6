...
public class TerrainDef : BuildableDef
{
	...
	public class SpaceEdgeGraphicData
	{
		...
		public void EnsureInitialized(TerrainDef def)
		{
			...
			oShape = new CachedMaterial(OShapeTexPath, shader2, (Color)(((_003F?)overrideColor) ?? def.color));
			uShape = new CachedMaterial(UShapeTexPath, shader2, (Color)(((_003F?)overrideColor) ?? def.color));
			cornerInner = new CachedMaterial(CornerInnerTexPath, shader2, (Color)(((_003F?)overrideColor) ?? def.color));
			cornerOuter = new CachedMaterial(CornerOuterTexPath, shader2, (Color)(((_003F?)overrideColor) ?? def.color));
			flat = new CachedMaterial(FlatTexPath, shader2, (Color)(((_003F?)overrideColor) ?? def.color));
			loopLeft = new CachedMaterial(LoopLeftTexPath, shader, (Color)(((_003F?)overrideColor) ?? def.color));
			loopRight = new CachedMaterial(LoopRightTexPath, shader, (Color)(((_003F?)overrideColor) ?? def.color));
			loopSingle = new CachedMaterial(LoopSingleTexPath, shader, (Color)(((_003F?)overrideColor) ?? def.color));
			foreach (string loopTexPath in LoopTexPaths)
			{
				loops.Add(new CachedMaterial(loopTexPath, shader, (Color)(((_003F?)overrideColor) ?? def.color)));
			}
		}
		...
		public Material GetMaterial(TerrainDef terrain, SectionLayer_TerrainEdges.EdgeType edgeType, int listIndexOffset = 0)
		{
			...
			return (Material)(edgeType switch
			{
		...
		LongEventHandler.ExecuteWhenFinished(delegate
		{
			...
			if (!dontRender)
			{
				if (graphic == BaseContent.BadGraphic)
				{
					...
					{
						graphic.MatSingle.SetTexture(ShaderPropertyIDs.AlphaAddTex, (Texture)(object)TexGame.AlphaAddTex);
					}
				...
				if (!waterDepthShader.NullOrEmpty())
				{
					...
					waterDepthMaterial.SetTexture(ShaderPropertyIDs.AlphaAddTex, (Texture)(object)TexGame.AlphaAddTex);
				...
				if (ModsConfig.BiotechActive && graphicPolluted == BaseContent.BadGraphic && (!pollutionOverlayTexturePath.NullOrEmpty() || !pollutedTexturePath.NullOrEmpty()))
				{
					...
					{
						matSingle.SetTexture(ShaderPropertyIDs.BurnTex, (Texture)(object)val);
					}
					matSingle.SetColor(ShaderPropertyIDs.BurnColor, pollutionColor);
					matSingle.SetVector(ShaderPropertyIDs.BurnScale, Vector4.op_Implicit(pollutionOverlayScale));
					matSingle.SetVector(ShaderPropertyIDs.ScrollSpeed, Vector4.op_Implicit(pollutionOverlayScrollSpeed));
					matSingle.SetColor(ShaderPropertyIDs.PollutionTintColor, pollutionTintColor);
					if (edgeType == TerrainEdgeType.FadeRough)
					{
						matSingle.SetTexture(ShaderPropertyIDs.AlphaAddTex, (Texture)(object)TexGame.AlphaAddTex);
					}
					if ((Object)(object)matSingle != (Object)(object)graphic.MatSingle)
					{
