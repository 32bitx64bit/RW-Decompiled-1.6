...
public class WindowStack
{
	...
	public void HandleEventsHighPriority()
	{
		...
		if ((int)Event.current.type == 0 && GetWindowAt(UI.GUIToScreenPoint(Event.current.mousePosition)) == null)
		{
		...
		if (((int)Event.current.type == 0 || (int)Event.current.type == 4) && !GetsInput(null))
		{
			Event.current.Use();
		}
	}
	...
	public Window GetWindowAt(Vector2 pos)
	{
		...
		for (int num = windows.Count - 1; num >= 0; num--)
		{
			if (((Rect)(ref windows[num].windowRect)).Contains(pos))
			{
				return windows[num];
			}
		}
