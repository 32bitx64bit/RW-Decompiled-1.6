...
public static class DebugActionsMisc
{
	...
	private static void HSVAtPosition()
	{
		...
		foreach (IntVec3 item in GenRadial.RadialCellsAround(UI.MouseCell(), 10f, useCenter: true))
		{
			if (item.InBounds(currentMap))
			{
				Color.RGBToHSV(Color32.op_Implicit(Find.CurrentMap.glowGrid.VisualGlowAt(item)), ref num, ref num2, ref num3);
				currentMap.debugDrawer.FlashCell(item, 0.5f, $"HSV({num:.0#},{num2:.0#},{num3:.0#})", 100);
			}
		}
	}
				...
				for (int k = 0; k < 100; k++)
				{
					...
					if (pawn2.royalty != null)
					{
						...
						if (mostSeniorTitle2 != null && !mostSeniorTitle2.def.requiredApparel.NullOrEmpty())
						{
							...
							{
								ApparelRequirement apparelRequirement = mostSeniorTitle2.def.requiredApparel[l];
								if (apparelRequirement.IsActive(pawn2) && !apparelRequirement.IsMet(pawn2))
								{
									Log.Error(localKindDef?.ToString() + " (" + mostSeniorTitle2.def.label + ")  does not have its title requirements met. index=" + l + logApparel(pawn2));
									flag = true;
								}
							}
						}
