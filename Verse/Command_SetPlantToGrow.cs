...
public class Command_SetPlantToGrow : Command
{
	...
	public Command_SetPlantToGrow()
	{
		...
		if (flag)
		{
			icon = (Texture)(object)SetPlantToGrowTex;
			defaultLabel = "CommandSelectPlantToGrowMulti".Translate();
			return;
		}
		icon = (Texture)(object)thingDef.uiIcon;
	...
	public override void ProcessInput(Event ev)
	{
		...
		for (int i = 0; i < tmpAvailablePlants.Count; i++)
		{
			...
			}, plantDef, null, forceBasicStyle: false, MenuOptionPriority.Default, null, null, 29f, (Rect rect) => Widgets.InfoCardButton(((Rect)(ref rect)).x + 5f, ((Rect)(ref rect)).y + (((Rect)(ref rect)).height - 24f) / 2f, plantDef)));
		}
		if (list.Any())
		{
