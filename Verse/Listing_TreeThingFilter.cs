...
public class Listing_TreeThingFilter : Listing_Tree
{
	...
	private void DoThingDef(ThingDef tDef, int nestLevel, Map map)
	{
		...
		if ((Object)(object)tDef.uiIcon != (Object)null && (Object)(object)tDef.uiIcon != (Object)(object)BaseContent.BadTex)
		{
			nestLevel++;
			Widgets.DefIcon(new Rect(XAtIndentLevel(nestLevel) - 6f, curY, 20f, 20f), tDef, null, 1f, null, drawPlaceholder: true, val);
		}
		if (CurrentRowVisibleOnScreen())
		{
			...
			LabelLeft(tDef.LabelCap, text, nestLevel, num2, val);
	...
	private bool CurrentRowVisibleOnScreen()
	{
		...
		return ((Rect)(ref visibleRect)).Overlaps(val);
	}
