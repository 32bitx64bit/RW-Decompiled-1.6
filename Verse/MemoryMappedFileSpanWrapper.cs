...
public class MemoryMappedFileSpanWrapper : IDisposable
{
	public class SimpleReadStream : Stream
	{
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			_mmf.GetSpan(_pos, count).CopyTo(MemoryExtensions.AsSpan<byte>(buffer, offset, count));
			_pos += count;
			return count;
		}
		...
		public virtual int Read(Span<byte> buffer)
		{
			...
			if (_pos + buffer.Length >= _mmf._length)
			{
				...
				Span<byte> val = buffer;
				buffer = val.Slice(0, num);
			}
	...
	private unsafe Span<byte> GetSpan(long offset, long length)
	{
		...
		return new Span<byte>((void*)(_pointer + offset), (int)length);
	}
	...
	public ReadOnlySpan<byte> GetReadOnlySpan(long offset, long length)
	{
		...
		return Span<byte>.op_Implicit(GetSpan(offset, length));
	}
	...
	{
		//IL_0009: Unknown result type (might be due to invalid IL or missing references)
		MemoryMarshal.Write<T>(GetSpan(offset, sizeof(T)), ref value);
	}
