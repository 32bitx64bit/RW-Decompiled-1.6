...
public class FactionSource : IPathFinderDataSource, IDisposable
{
	...
	public ReadOnly<ushort> this[Faction faction]
	{
		get
		{
	...
	public FactionSource(Map map)
	{
		...
		noCost = new NativeArray<ushort>(cellCount, (Allocator)4, (NativeArrayOptions)1);
	}
	...
	private void ComputeAllForFaction(Faction faction)
	{
		...
		if (!factionCosts.TryGetValue(faction, out var value))
		{
			value._002Ector(cellCount, (Allocator)4, (NativeArrayOptions)1);
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				int num2 = cellIndices.CellToIndex(item);
				ref NativeArray<ushort> reference = ref value;
				int num3 = num2;
				reference[num3] += num;
			}
		}
		...
		for (int j = 0; j < count2; j++)
		{
			...
			ushort num4 = PathFindCost(frame, faction);
			foreach (IntVec3 item2 in frame.OccupiedRect())
			{
				int num5 = cellIndices.CellToIndex(item2);
				ref NativeArray<ushort> reference = ref value;
				int num3 = num5;
				reference[num3] += num4;
			}
