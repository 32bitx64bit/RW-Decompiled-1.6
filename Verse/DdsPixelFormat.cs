...
public struct DdsPixelFormat
{
	...
	public bool IsCompressed
	{
		get
		{
			if ((Flags & DdsPixelFormatFlags.FourCC) != 0)
			{
				return FourCC != 0;
			}
			return false;
	...
	public TextureFormat ToTextureFormat()
	{
		...
		if ((Flags & DdsPixelFormatFlags.RGB) != 0)
		{
			...
			if (IsRgb888 || IsBgr888)
			{
				...
				return (TextureFormat)3;
			}
			if (IsRgb565)
			{
				return (TextureFormat)7;
			}
			if (IsRgba4444)
			{
				if (!flag)
		...
		if ((Flags & DdsPixelFormatFlags.Luminance) != 0 || (Flags & DdsPixelFormatFlags.Alpha) != 0)
		{
