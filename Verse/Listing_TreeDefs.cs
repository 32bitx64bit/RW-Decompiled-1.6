...
public class Listing_TreeDefs : Listing_Tree
{
	...
	protected void ValueEditWidgetRight(TreeNode_Editor node, float leftX)
	{
		...
		((Rect)(ref rect))._002Ector(leftX, curY, base.ColumnWidth - leftX, lineHeight);
		...
		else if (objectType == typeof(bool))
		{
			...
			Widgets.Checkbox(new Vector2(((Rect)(ref rect)).x, ((Rect)(ref rect)).y), ref checkOn, lineHeight);
			obj = checkOn;
		}
		...
		else if (objectType == typeof(FloatRange))
		{
			...
			Widgets.FloatRangeWithTypeIn(rect, ((object)(Rect)(ref rect)).GetHashCode(), ref fRange, sliderMin, sliderMax);
			obj = fRange;
		}
		else if (objectType == typeof(IntRange))
		{
			...
			Widgets.IntRange(rect, ((object)(Rect)(ref rect)).GetHashCode(), ref range, min, max);
			obj = range;
		}
		else
		{
