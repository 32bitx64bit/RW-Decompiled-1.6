...
public static class GizmoGridDrawer
{
	...
	public static void DrawGizmoGrid(IEnumerable<Gizmo> gizmos, float startX, out Gizmo mouseoverGizmo, Func<Gizmo, bool> customActivatorFunc = null, Func<Gizmo, bool> highlightFunc = null, Func<Gizmo, bool> lowlightFunc = null, bool multipleSelected = false)
	{
		...
		((Vector2)(ref val))._002Ector(startX, num2);
		...
		for (int k = 0; k < gizmoGroups.Count; k++)
		{
			...
			if (gizmo2 != null)
			{
				...
				if (val.x + gizmo2.GetWidth(maxWidth) > num)
				{
					val.x = startX;
					val.y -= 75f + GizmoSpacing.y;
					num3++;
				}
				val.x += gizmo2.GetWidth(maxWidth) + GizmoSpacing.x;
		...
		((Vector2)(ref val2))._002Ector(startX, num2);
		mouseoverGizmo = null;
		...
		for (int num5 = 0; num5 < firstGizmos.Count; num5++)
		{
			...
			if (val2.x + gizmo3.GetWidth(maxWidth) > num)
			{
				val2.x = startX;
				val2.y -= 75f + GizmoSpacing.y;
			}
			heightDrawnFrame = Time.frameCount;
			heightDrawn = (float)UI.screenHeight - val2.y;
			...
			GizmoResult result2 = gizmo3.GizmoOnGUI(val2, maxWidth, parms);
			ProcessGizmoState(gizmo3, result2, ref mouseoverGizmo);
			isFirst = false;
			GenUI.AbsorbClicksInRect(new Rect(val2.x - 12f, val2.y, gizmo3.GetWidth(maxWidth) + 12f, 75f + GizmoSpacing.y));
			val2.x += gizmo3.GetWidth(maxWidth) + GizmoSpacing.x;
		}
		float x = val2.x;
		int num7 = 0;
		for (int num8 = 0; num8 < shrinkableCommands.Count; num8++)
		{
			...
			if (val2.x + getShrunkSize > num)
			{
				...
				val2.x = x;
				val2.y -= getShrunkSize + 3f;
			}
			Vector2 val3 = val2;
			val3.y += getShrunkSize + 3f;
			heightDrawnFrame = Time.frameCount;
			heightDrawn = Mathf.Min(heightDrawn, (float)UI.screenHeight - val3.y);
			...
			GizmoResult result3 = command2.GizmoOnGUIShrunk(val3, getShrunkSize, parms2);
			ProcessGizmoState(command2, result3, ref mouseoverGizmo);
			isFirst = false;
			GenUI.AbsorbClicksInRect(new Rect(val3.x - 3f, val3.y, getShrunkSize + 3f, getShrunkSize + 3f));
			val2.x += getShrunkSize + 3f;
		}
