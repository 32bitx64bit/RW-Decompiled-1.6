...
public static class GenCollection
{
	...
	public static void SortByColor<T>(this List<T> colorDefs, Func<T, Color> getColor)
	{
		colorDefs.SortBy(delegate(T x)
		{
			...
			Color.RGBToHSV(getColor(x), ref num4, ref num5, ref num6);
			return (!Mathf.Approximately(num5, 0f)) ? ((float)Mathf.RoundToInt(num4 * 100f)) : (-1f);
		}, delegate(T x)
		{
			//IL_0007: Unknown result type (might be due to invalid IL or missing references)
			...
			Color.RGBToHSV(getColor(x), ref num, ref num2, ref num3);
