...
public class DefInjectionPackage
{
	...
	public void AddDataFromFile(VirtualFile file, out bool xmlParseError, string preloadedFileContents)
	{
		...
		try
		{
			foreach (XElement item in ((XContainer)VirtualFileInfoExt.LoadAsXDocument(preloadedFileContents).Root).Elements())
			{
				if (item.Name == XName.op_Implicit("rep"))
				{
					string key = ProcessedPath(((XContainer)item).Elements(XName.op_Implicit("path")).First().Value);
					string translation = ProcessedTranslation(((XContainer)item).Elements(XName.op_Implicit("trans")).First().Value);
					TryAddInjection(file, key, translation);
					usedOldRepSyntax = true;
					continue;
				}
				string text = ProcessedPath(((object)item.Name).ToString());
				...
				else if (item.HasElements)
				{
					...
					bool flag = false;
					foreach (XNode item2 in ((XContainer)item).DescendantNodes())
					{
						XElement val = (XElement)(object)((item2 is XElement) ? item2 : null);
						if (val != null)
						{
							if (val.Name == XName.op_Implicit("li"))
							{
								list.Add(ProcessedTranslation(val.Value));
							}
							else if (!flag)
							{
						...
						XComment val2 = (XComment)(object)((item2 is XComment) ? item2 : null);
						if (val2 != null)
						{
							if (list2 == null)
							{
							...
							list2.Add(new Pair<int, string>(list.Count, val2.Value));
						}
					}
