...
public struct ShotReport
{
	...
	{
		float num = (acc.HasValue ? acc.Value : ((caster is Pawn) ? caster.GetStatValue(StatDefOf.ShootingAccuracyPawn) : (caster?.GetStatValue(StatDefOf.ShootingAccuracyTurret) ?? 1f)));
		float num2 = Mathf.Pow(num, distance);
		if (caster is Pawn)
		{
			num2 = ((distance <= 3f) ? (num2 * caster.GetStatValue(StatDefOf.ShootingAccuracyFactor_Touch)) : ((distance <= 12f) ? (num2 * Mathf.Lerp(caster.GetStatValue(StatDefOf.ShootingAccuracyFactor_Touch), caster.GetStatValue(StatDefOf.ShootingAccuracyFactor_Short), (distance - 3f) / 9f)) : ((distance <= 25f) ? (num2 * Mathf.Lerp(caster.GetStatValue(StatDefOf.ShootingAccuracyFactor_Short), caster.GetStatValue(StatDefOf.ShootingAccuracyFactor_Medium), (distance - 12f) / 13f)) : ((!(distance <= 40f)) ? (num2 * caster.GetStatValue(StatDefOf.ShootingAccuracyFactor_Long)) : (num2 * Mathf.Lerp(caster.GetStatValue(StatDefOf.ShootingAccuracyFactor_Medium), caster.GetStatValue(StatDefOf.ShootingAccuracyFactor_Long), (distance - 25f) / 15f))))));
		}
		return Mathf.Max(num2, 0.0201f);
	}
