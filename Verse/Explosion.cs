...
public class Explosion : Thing
{
	...
	public int GetDamageAmountAt(IntVec3 c)
	{
		...
		float num = c.DistanceTo(base.Position) / radius;
		return Mathf.Max(GenMath.RoundRandom(Mathf.Lerp((float)damAmount, (float)damAmount * 0.2f, num)), 1);
	}
	...
	public float GetArmorPenetrationAt(IntVec3 c)
	{
		...
		float num = c.DistanceTo(base.Position) / radius;
		return Mathf.Lerp(armorPenetration, armorPenetration * 0.2f, num);
	}
