...
public class Verb_LaunchProjectile : Verb
{
	...
	private static IEnumerable<IntVec3> GenerateEvenDispersalForcedMissTargets(IntVec3 root, float radius, int count)
	{
		...
		{
			float num = MathF.PI * 2f * (float)i / goldenRatio;
			float num2 = Mathf.Acos(1f - 2f * ((float)i + 0.5f) / (float)count);
			int num3 = (int)(Mathf.Cos(num) * Mathf.Sin(num2) * radius);
			int num4 = (int)(Mathf.Cos(num2) * radius);
			Vector3 vect = Vector3Utility.RotatedBy(new Vector3((float)num3, 0f, (float)num4), randomRotationOffset);
