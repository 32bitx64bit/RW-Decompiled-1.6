...
public struct CellRect : IEquatable<CellRect>, IEnumerable<IntVec3>, IEnumerable
{
	...
	public bool Overlaps(Bounds other)
	{
		...
		if (IsEmpty || ((Bounds)(ref other)).size == Vector3.zero)
		{
			return false;
		}
		if ((float)minX <= ((Bounds)(ref other)).max.x && (float)maxX >= ((Bounds)(ref other)).min.x && (float)maxZ >= ((Bounds)(ref other)).min.z)
		{
			return (float)minZ <= ((Bounds)(ref other)).max.z;
		}
		return false;
	}
	...
	public Bounds ToBounds()
	{
		...
		((Bounds)(ref result)).center = CenterVector3;
		((Bounds)(ref result)).size = new Vector3((float)Width, 1f, (float)Height);
		return result;
	}
	...
	public Rect ToScreenRect(Camera camera)
	{
		...
		Vector3 val = camera.WorldToScreenPoint(Min.ToVector3());
		Vector3 val2 = camera.WorldToScreenPoint(Max.ToVector3() + new Vector3(1f, 0f, 1f));
	...
	public void DebugDraw()
	{
		...
		((Vector3)(ref val))._002Ector((float)minX, num, (float)minZ);
		Vector3 val2 = default(Vector3);
		((Vector3)(ref val2))._002Ector((float)minX, num, (float)(maxZ + 1));
		Vector3 val3 = default(Vector3);
		((Vector3)(ref val3))._002Ector((float)(maxX + 1), num, (float)(maxZ + 1));
		Vector3 val4 = default(Vector3);
		((Vector3)(ref val4))._002Ector((float)(maxX + 1), num, (float)minZ);
		GenDraw.DrawLineBetween(val, val2);
		GenDraw.DrawLineBetween(val2, val3);
		GenDraw.DrawLineBetween(val3, val4);
		GenDraw.DrawLineBetween(val4, val);
	}
