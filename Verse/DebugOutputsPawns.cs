...
public static class DebugOutputsPawns
{
	...
	public static void Mechanoids()
	{
		...
		{
			RecipeDef recipeDef5 = recipeFor(k);
			return (recipeDef5 == null) ? "-" : recipeDef5.gestationCycles.ToString();
		};
		Func<PawnKindDef, float> formingTime = delegate(PawnKindDef k)
		{
			RecipeDef recipeDef4 = recipeFor(k);
			return (recipeDef4 != null) ? ((float)(recipeDef4.formingTicks * recipeDef4.gestationCycles) / 60000f) : 0f;
		};
		...
		Func<PawnKindDef, string> ingredientMarketValue = delegate(PawnKindDef k)
		{
			...
			float num = 0f;
			foreach (IngredientCount ingredient in recipeDef2.ingredients)
			{
				Thing thing = ThingMaker.MakeThing(ingredient.FixedIngredient);
				thing.stackCount = ingredient.CountRequiredOfFor(ingredient.FixedIngredient, recipeDef2);
		...
		{
			Pawn pawn3 = PawnGenerator.GeneratePawn(new PawnGenerationRequest(k));
			ThingWithComps primary2 = pawn3.equipment.Primary;
			ThingDef result3 = null;
			if (primary2 != null && primary2.def.IsWeapon)
			{
				result3 = primary2.def;
			}
		...
		{
			Pawn pawn2 = PawnGenerator.GeneratePawn(new PawnGenerationRequest(k));
			float result2 = 0f;
			ThingWithComps primary = pawn2.equipment.Primary;
			if (primary != null && primary.def.IsRangedWeapon)
			{
				result2 = primary.def.Verbs[0].GetHitChanceFactor(primary, distance);
			}
