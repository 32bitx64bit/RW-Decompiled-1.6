...
public class DamageWorker_AddInjury : DamageWorker
{
	...
	private DamageResult ApplyToPawn(DamageInfo dinfo, Pawn pawn)
	{
		...
		if (dinfo.ApplyAllDamage)
		{
			...
			int num2 = 25;
			float num3 = num / (float)dinfo.DamagePropagationPartsRange.RandomInRange;
			do
			{
				DamageInfo dinfo2 = dinfo;
				dinfo2.SetAmount(Mathf.Min(num, num3));
		...
		{
			MoteMaker.ThrowText(new Vector3((float)pawn.Position.x + 1f, (float)pawn.Position.y, (float)pawn.Position.z + 1f), pawn.Map, "Headshot".Translate(), Color.white);
	...
	protected float FinalizeAndAddInjury(Pawn pawn, Hediff_Injury injury, DamageInfo dinfo, DamageResult result)
	{
		...
		if (pawn.IsColonist && !dinfo.IgnoreInstantKillProtection && dinfo.Def.ExternalViolenceFor(pawn) && !Rand.Chance(Find.Storyteller.difficulty.allowInstantKillChance))
		{
			...
			for (int i = 0; i < 7; i++)
			{
				...
				float num4 = Mathf.Clamp(partHealth - num, num2, num3);
				if (DebugViewSettings.logCauseOfDeath)
				{
					Log.Message($"CauseOfDeath: attempt to prevent death for {pawn.Name} on {injury.Part.Label} attempt:{i + 1} severity:{injury.Severity}->{num4} part health:{partHealth}");
				}
				injury.Severity = num4;
				num *= 2f;
				num2 = 0f;
			}
		}
		pawn.health.AddHediff(injury, null, dinfo, result);
		float num5 = Mathf.Min(injury.Severity, partHealth);
		result.totalDamageDealt += num5;
		result.wounded = true;
