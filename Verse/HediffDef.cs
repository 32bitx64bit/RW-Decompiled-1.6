...
public class HediffDef : Def, IRenderNodePropertiesParent
{
	...
	public override IEnumerable<string> ConfigErrors()
	{
		...
		{
			for (int m = 0; m < possiblePathways.Count - 1; m++)
			{
				HediffInfectionPathway vector = possiblePathways[m];
				for (int num = m + 1; num < possiblePathways.Count; num++)
				{
					HediffInfectionPathway hediffInfectionPathway = possiblePathways[num];
		...
		if (stages != null)
		{
			...
			for (int m = 0; m < stages.Count; m++)
			{
				...
				{
					for (int n = 0; n < stages[m].hediffGivers.Count; n++)
					{
						foreach (string item3 in stages[m].hediffGivers[n].ConfigErrors())
						{
							yield return item3;
						}
					}
				}
				if (stages[m].minSeverity > maxSeverity)
				{
					yield return $"minSeverity of stage {m} is greater than maxSeverity.";
				}
			}
		}
