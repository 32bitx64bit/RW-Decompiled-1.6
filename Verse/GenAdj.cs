...
public static class GenAdj
{
	...
	public static IEnumerable<IntVec3> CellsAdjacentAlongEdge(IntVec3 center, Rot4 rot, IntVec2 size, int dir)
	{
		...
		if (dir == 0)
		{
			...
			{
				yield return new IntVec3(x4, center.y, maxZ + 1);
			}
		}
		if (dir == 1)
		{
			...
			{
				yield return new IntVec3(maxX + 1, center.y, x4);
			}
		}
		if (dir == 2)
		{
			for (int x4 = minX; x4 < maxX; x4++)
			{
				yield return new IntVec3(x4, center.y, minZ - 1);
			}
		}
		if (dir == 3)
		{
			for (int x4 = minZ; x4 < maxZ; x4++)
			{
				yield return new IntVec3(minX - 1, center.y, x4);
			}
		}
	}
	...
	public static Bounds DrawBounds(this Thing t)
	{
		...
		Vector3 val2 = t.Rotation.AsQuat * t.def.size.ToVector3();
