...
public static class DebugOutputsInfection
{
	...
	public static void Infections()
	{
		...
		{
			float num3 = (isAnimal(d) ? 1f : ThingDefOf.Bed.GetStatValueAbstract(StatDefOf.ImmunityGainSpeedFactor));
			float num4 = Mathf.Lerp(1f, num3, bedridden ? 1f : 0.3f) * StatDefOf.ImmunityGainSpeed.GetStatPart<StatPart_Resting>().factor;
