...
public static class WorldPollutionUtility
{
	...
	public static void PolluteWorldAtTile(PlanetTile root, float pollutionAmount)
	{
		...
		if (planetTile.Valid)
		{
			...
			{
				Vector2 val = Find.WorldGrid.LongLatOf(planetTile);
				string text = val.y.ToStringLatitude() + " / " + val.x.ToStringLongitude();
	...
	public static float CalculateNearbyPollutionScore(PlanetTile tileId)
	{
		...
		{
			nearbyPollutionScore += NearbyPollutionOverDistanceCurve.Evaluate(Mathf.RoundToInt((float)dist)) * Find.WorldGrid[tile].pollution;
			return false;
		}, maxTilesToProcess);
