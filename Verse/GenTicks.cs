...
public static class GenTicks
{
	...
	public static int ConfiguredTicksAbsAtGameStart
	{
		get
		{
			...
			Vector2 val = ((!gameInitData.startingTile.Valid) ? Vector2.zero : Find.WorldGrid.LongLatOf(gameInitData.startingTile));
			Twelfth twelfth = ((gameInitData.startingSeason != 0) ? gameInitData.startingSeason.GetFirstTwelfth(val.y) : ((!gameInitData.startingTile.Valid) ? Season.Summer.GetFirstTwelfth(0f) : TwelfthUtility.FindStartingWarmTwelfth(gameInitData.startingTile)));
