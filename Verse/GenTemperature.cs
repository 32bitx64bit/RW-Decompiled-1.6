...
public static class GenTemperature
{
	...
	public static float ControlTemperatureTempChange(IntVec3 cell, Map map, float energyLimit, float targetTemperature)
	{
		...
		float num = energyLimit / (float)room.CellCount;
		float num2 = targetTemperature - room.Temperature;
		float num3 = 0f;
		if (energyLimit > 0f)
		{
			num3 = Mathf.Min(num2, num);
			return Mathf.Max(num3, 0f);
		}
		num3 = Mathf.Max(num2, num);
		return Mathf.Min(num3, 0f);
	}
