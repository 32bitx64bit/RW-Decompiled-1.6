...
public class SectionLayer_LightingOverlay : SectionLayer
{
	...
	public string GlowReportAt(IntVec3 c)
	{
		...
		Color32 val = colors[botLeft];
		stringBuilder.Append("BL=" + ((object)(Color32)(ref val)).ToString());
		val = colors[topLeft];
		stringBuilder.Append("\nTL=" + ((object)(Color32)(ref val)).ToString());
		val = colors[topRight];
		stringBuilder.Append("\nTR=" + ((object)(Color32)(ref val)).ToString());
		val = colors[botRight];
		stringBuilder.Append("\nBR=" + ((object)(Color32)(ref val)).ToString());
		val = colors[center];
		stringBuilder.Append("\nCenter=" + ((object)(Color32)(ref val)).ToString());
	...
	private static void GenerateLightingOverlay(Map map, LayerSubMesh subMesh, CellRect rect, ref int firstCenterInd, bool centered = false, Predicate<int> filter = null)
	{
		...
		Color32[] array = (Color32[])(object)new Color32[subMesh.verts.Count];
		int maxX = rect.maxX;
		int maxZ = rect.maxZ;
		...
		for (int i = rect.minZ; i <= maxZ + 1; i++)
		{
			...
			while (num4 <= maxX + 1)
			{
				...
				{
					array[botLeft] = new Color32((byte)0, (byte)0, (byte)0, (byte)0);
				}
				else
				{
		...
		for (int k = rect.minZ; k <= maxZ; k++)
		{
			...
			while (num9 <= maxX)
			{
				...
				{
					array[center2] = new Color32((byte)0, (byte)0, (byte)0, (byte)0);
				}
				else
				{
	...
	private static void MakeBaseGeometry(Map map, LayerSubMesh sm, CellRect sectRect, out int firstCenterInd, bool centered = false)
	{
		...
		for (int k = sectRect.minZ; k <= sectRect.maxZ; k++)
		{
			...
			{
				sm.verts.Add(new Vector3((float)l + num + 0.5f, num3, (float)k + num2 + 0.5f));
			}
		}
