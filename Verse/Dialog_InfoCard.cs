...
public class Dialog_InfoCard : Window
{
	...
	private static bool ShowMaterialsButton(Rect containerRect, bool withBackButtonOffset = false)
	{
		...
		float num = ((Rect)(ref containerRect)).x + ((Rect)(ref containerRect)).width - 14f - 200f - 16f;
		if (withBackButtonOffset)
		{
			num -= 136f;
		}
		return Widgets.ButtonText(new Rect(num, ((Rect)(ref containerRect)).y + 18f, 200f, 40f), "ShowMaterials".Translate());
	}
	...
	public override void DoWindowContents(Rect inRect)
	{
		...
		((Rect)(ref rect)).height = 34f;
		((Rect)(ref rect)).x = ((Rect)(ref rect)).x + 34f;
		Text.Font = GameFont.Medium;
		Widgets.Label(rect, GetTitle());
		Rect rect2 = default(Rect);
		((Rect)(ref rect2))._002Ector(((Rect)(ref inRect)).x + 9f, ((Rect)(ref rect)).y, 34f, 34f);
		if (thing != null)
		{
		...
		((Rect)(ref val)).yMin = ((Rect)(ref rect)).yMax;
		((Rect)(ref val)).yMax = ((Rect)(ref val)).yMax - 38f;
		Rect val2 = val;
		...
		if (list.Count > 1)
		{
			((Rect)(ref val2)).yMin = ((Rect)(ref val2)).yMin + 45f;
			TabDrawer.DrawTabs(val2, list);
		}
		FillCard(val2.ContractedBy(18f));
