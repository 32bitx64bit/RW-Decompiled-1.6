...
public static class ShaderDatabase
{
	...
	public static void LoadShader(string shaderPath, string uiShaderPath, out Shader result, out Shader uiShader)
	{
		...
		if (TryLoadShader(shaderPath, out result) && (Object)(object)uiShader != (Object)null)
		{
	...
	private static bool TryLoadShader(string shaderPath, out Shader result)
	{
		...
		{
			Object obj = Resources.Load("Materials/" + shaderPath, typeof(Shader));
			Shader val = (Shader)(object)((obj is Shader) ? obj : null);
			if (val != null)
			{
				lookup[shaderPath] = val;
			}
			else
			{
				Shader val2 = ContentFinder<Shader>.TryFindAssetInModBundles(shaderPath);
				if (val2 != null)
				{
					lookup[shaderPath] = val2;
				}
				else
				{
