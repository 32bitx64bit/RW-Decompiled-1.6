...
public class DamageWorker
{
	...
	public virtual void ExplosionStart(Explosion explosion, List<IntVec3> cellsToAffect)
	{
		...
		if (explosion.doVisualEffects)
		{
			...
			{
				Vector3 val = explosion.Position.ToVector3Shifted() - ((Component)Find.Camera).transform.position;
	...
	protected virtual void ExplosionVisualEffectCenter(Explosion explosion)
	{
		...
		for (int j = 0; j < num; j++)
		{
			Vector3 val = Gen.RandomHorizontalVector(explosion.radius * def.explosionInteriorCellDistanceMultiplier);
			if (def.explosionInteriorEffecter != null)
			{
				Vector3 vect = explosion.Position.ToVector3Shifted() + val;
			...
			{
				FleckMaker.ThrowExplosionInterior(explosion.Position.ToVector3Shifted() + val, explosion.Map, def.explosionInteriorFleck);
			}
			else
			{
				MoteMaker.ThrowExplosionInteriorMote(explosion.Position.ToVector3Shifted() + val, explosion.Map, def.explosionInteriorMote);
			}
		}
	}
	...
	public virtual void ExplosionAffectCell(Explosion explosion, IntVec3 c, List<Thing> damagedThings, List<Thing> ignoredThings, bool canThrowMotes)
	{
		...
		{
			float num = Mathf.Clamp01((explosion.Position - c).LengthHorizontal / explosion.radius);
			Color color = Color.Lerp(def.explosionColorCenter, def.explosionColorEdge, num);
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			if (thing.def.category != ThingCategory.Mote && thing.def.category != ThingCategory.Ethereal)
			{
				...
				if (thing.def.Fillage == FillCategory.Full && thing.def.Altitude > num2)
				{
					flag = true;
					num2 = thing.def.Altitude;
				}
			}
		}
		for (int j = 0; j < thingsToAffect.Count; j++)
		{
			if (thingsToAffect[j].def.Altitude >= num2)
			{
		...
		if (def.explosionSnowMeltAmount > 0.0001f)
		{
			...
			float num3 = 1f - lengthHorizontal / explosion.radius;
			if (num3 > 0f)
			{
				explosion.Map.snowGrid.AddDepth(c, (0f - num3) * def.explosionSnowMeltAmount);
			}
		}
	...
	public virtual IEnumerable<IntVec3> ExplosionCellsToHit(IntVec3 center, Map map, float radius, IntVec3? needLOSToCell1 = null, IntVec3? needLOSToCell2 = null, FloatRange? affectedAngle = null)
	{
		...
		for (int i = 0; i < num3; i++)
		{
			...
			if (affectedAngle.HasValue)
			{
				...
				float num5 = Mathf.Atan2((float)(-(intVec.z - center.z)), (float)(intVec.x - center.x)) * 57.29578f;
				float num6 = num;
				float num7 = num2;
