...
public class Pawn_AgeTracker : IExposable
{
	...
	public float BiologicalTicksPerTick
	{
		get
		{
			...
			{
				float num2 = Mathf.InverseLerp(11f, 20f, ageBiologicalYearsFloat);
				num *= Mathf.Lerp(ChildAgingMultiplier, AdultAgingMultiplier, num2);
			}
			if (pawn.genes != null)
			{
	...
	private void CalculateGrowth(int interval)
	{
		...
		growth += PawnUtility.BodyResourceGrowthSpeed(pawn) * (float)interval / Mathf.Max((float)AdultMinAgeTicks, 1f);
