...
public static class PollutionUtility
{
	...
	public static void GrowPollutionAt(IntVec3 root, Map map, int cellCountToPollute = 4, Action<IntVec3> onPolluteAction = null, bool silent = false, Func<IntVec3, bool> adjacentPolluteValidator = null)
	{
		...
		else
		{
			...
			{
				foreach (IntVec3 adjacentPollutableCell in GetAdjacentPollutableCells(tmpPollutedCells[i], map))
				{
					if (!tmpPollutableCells.Contains(adjacentPollutableCell))
					{
						tmpPollutableCells.Push(adjacentPollutableCell);
					}
				}
			}
			...
			while (cellCountToPollute > 0 && tmpPollutableCells.Count > 0)
			{
				...
				foreach (IntVec3 adjacentPollutableCell2 in GetAdjacentPollutableCells(intVec, map))
				{
					if (!tmpPollutableCells.Contains(adjacentPollutableCell2))
					{
						tmpPollutableCells.Push(adjacentPollutableCell2);
					}
				}
				cellCountToPollute--;
			}
